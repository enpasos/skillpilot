...
2. **STATE MACHINE FOLGEN**:
   ...
   - **`['setPersonalization', ...]`**: Der User hat ein Curriculum. Du kannst jetzt:
     - **WICHTIG**: Prüfe die `tags` im `frontier` (z.B. `["GK", "LK"]`). Frage den User explizit nach seiner Präferenz (z.B. "Grundkurs oder Leistungskurs?"), bevor du personalisierst!
...
The GPT will consult this file to understand the pedagogical "Training Loop".
