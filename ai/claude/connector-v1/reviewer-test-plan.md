...
| --- | --- | --- |
| `get_skillpilot_coach_context` | Load the reset profile in DE and EN with valid Mathematics and Physics plans. | Inspector sees the bounded current curriculum/state payload, including the backend-formulated `learningPlanToday.text`, the period basis, the evaluability and status direction, one non-numeric row per subject and an unavailable-plan count, without plan counts, plan/landscape IDs, permanent ID, token, answer key or unrestricted state dump. Hosted Claude quotes that text verbatim for both subjects before active-goal coaching and adds no counts or judgement of its own. |
