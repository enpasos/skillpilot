...
| --- | --- | --- |
| `get_skillpilot_coach_context` | Load the reset profile in DE and EN with valid Mathematics and Physics plans. | Inspector sees the bounded current curriculum/state payload, including the backend-formulated `learningPlanToday.text`, with an active goal the separate `activeGoalAnnouncement`, the period basis, the evaluability and status direction, one non-numeric row per subject and an unavailable-plan count, without plan counts, plan/landscape IDs, permanent ID, token, answer key or unrestricted state dump. Hosted Claude quotes that text verbatim for both subjects, adds no counts or judgement of its own and then announces the active goal once before coaching it. |
...
| `set_skillpilot_mastery` | In Hosted Claude, complete the prepared ordinary exercise with visible evidence, then let Claude send only structured completion and concurrency data. Assessment reasoning and learner feedback stay in the conversation. In Inspector, exercise the closed schema and canonical write independently. | Hosted Claude records completion only after it has semantically checked suitable evidence. Inspector proves that `workFeedback` and `outcomeFeedback` are absent from the schema and rejected before core access, no model-selected numeric mastery is accepted, and one valid write advances once and is visible after reload. The returned `1.0` is a binary completion marker, not a grade; visible feedback stays learner-friendly. |
...
| `record_skillpilot_verified_recall_results` | Submit exactly one ordered `{cardId, passed}` result for every graded card, current revision and a fresh request ID. Keep all answers, assessment reasoning and feedback in Claude. | One atomic write, no partial scheduling update, correct next continuation and no separate memory-mastery write. Nested feedback or other unsupported fields are rejected before core access and replay hashing. No new `lastFeedback` is persisted. Hosted Claude follows the continuation without exposing capabilities or state fields. |
