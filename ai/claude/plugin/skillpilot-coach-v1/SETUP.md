...
1. On an eligible paid Claude account, add the authorized SkillPilot Marketplace
   ...
   **SkillPilot Coach v1** 1.1.7 candidate and enable it.
   ...
   This privacy correction requires the corrected backend and a refreshed
   connector tool catalog. Its mastery and Verified Recall inputs no longer
   accept assessment prose; old cached schemas are not compatible. Feedback
   and success messages are generated by Claude in the chat, not sent to Core.
