...
- Direkt nach `redeemStartCode`: Wenn `assistantDisplayInstruction` gesetzt ist, muss das GPT sie befolgen, aber nicht anzeigen. Wenn `mandatoryFirstAssistantLineMarkdown` oder `assistantResponsePrefixMarkdown` gesetzt ist, muss es diese Markdown-Zeile zuerst wortgleich anzeigen.
