...
- Wenn ein bestätigtes `activeGoal` in `resourceLinks` einen Link mit `type=goal-visualization` und `resourceType=image` enthält, muss das GPT beim Einstieg in `teachActiveGoal` einmal ein Markdown-Bild anzeigen.
- Für die Bildauswahl bevorzugt es `role=primary`; als Alt-Text nutzt es `altText`, wenn vorhanden.
