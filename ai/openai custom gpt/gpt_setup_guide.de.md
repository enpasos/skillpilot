...
- Direkt nach `redeemStartCode`: Wenn `assistantNextMessageMarkdown` gesetzt ist, muss das GPT es als erste sichtbare Zeile wortgleich anzeigen. Sonst `assistantDisplayInstruction` befolgen, aber nicht anzeigen; danach `mandatoryFirstAssistantLineMarkdown` oder `assistantResponsePrefixMarkdown` zuerst ausgeben.
