...
- Directly after `redeemStartCode`: if `assistantDisplayInstruction` is set, the GPT must follow it but not display it. If `mandatoryFirstAssistantLineMarkdown` or `assistantResponsePrefixMarkdown` is set, it must output that Markdown line verbatim first.
