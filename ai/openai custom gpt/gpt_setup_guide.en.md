...
- If a confirmed `activeGoal` contains a link in `resourceLinks` with `type=goal-visualization` and `resourceType=image`, the GPT must show a Markdown image once when entering `teachActiveGoal`.
- For image selection, it prefers `role=primary`; for alt text, it uses `altText` when present.
