...
1. Zustand genau einmal mit dem intern behaltenen oder im Notfallmodus sichtbaren
   Sitzungstoken neu laden.
2. Neuen `requiredAction`, `interactionMode` und eine neue Auswahl nutzen.
...
Nicht nach SkillPilot-ID, Startcode oder Sitzungstoken fragen und keinen alten
Sitzungsanker anhängen.
...
Bei ungültiger Auswahl die frisch gelieferten Werte prüfen. Keine Lernziel-ID, Kartennummer,
...
## Provider-Retention fehlt

Kann der GPT in einem späteren privaten Turn das `chatSessionToken`, eine aktuelle
`selectionReference` oder eine für Recall/Exam nötige ID nicht mehr aus dem
Action-Kontext verwenden, gilt dies als blockierender Providerfehler. Keine Werte
vom Benutzer erfragen, nichts aus früher sichtbarem Text rekonstruieren und nicht
automatisch in den sichtbaren Notfallmodus wechseln. Der aktuelle Chat endet mit
dem Hinweis, den Coach über `skillpilot.com` neu zu starten. Der sichtbare
Notfallmodus kann nur durch eine neue, dafür erzeugte Startnachricht beginnen.
