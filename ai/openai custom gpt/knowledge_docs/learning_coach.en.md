...
- If the state returns `chooseMemoryMode`, the next step is flashcard mode: practice in the cockpit or hard verification through Verified Recall. If no preference is clear, briefly ask the learner to choose between both modes. No generic "Start Exercise".
