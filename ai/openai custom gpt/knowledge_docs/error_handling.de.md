...
Wenn `verified-recall/start` danach `status=waiting` liefert, ist heute keine
weitere harte Kartenprüfung möglich. `getLearnerState` neu laden; das Backend
liefert das Lernkarten-Ziel dann nicht mehr als `activeGoal` oder `goalOptions`.
Wenn die lernende Person ein anderes Ziel möchte, aus diesem neu geladenen
Zustand mit `setActiveGoal` ein anderes atomares Frontier-Ziel wählen.
