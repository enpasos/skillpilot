...
This package now tests two observable questions:

> Does the Custom GPT Action flow preserve fresh values from a successful HTTP 200 JSON response into an immediately following Action call within the same assistant turn?

> Are selected values from that successful Action response available again after the immediately following user-message boundary in the same short conversation?
...
For the cross-turn test, `RETAIN_READY` does not prove that the value was durably stored. A following `RETAIN_MISSING` proves only that the earlier value was not used in the visible response. It does not prove that OpenAI deleted the raw result, that every Action field is affected, or that context compaction is the cause. Those are questions for OpenAI's internal traces. The test value is synthetic and has no authentication or authorization effect, so the architectural question is not limited to credentials: it concerns the supported lifetime of ordinary backend response data.
