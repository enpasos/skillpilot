...
Getestet werden sowohl die beobachtbare ChatGPT-Action-Kette innerhalb eines Assistant-Turns als auch die Wiederverwendung eines Action-Ergebnisses nach der unmittelbar folgenden Nutzer-Turn-Grenze:
...
```text
GET  https://skillpilot.com/openai/custom-gpt-action-regression
...
Die erste URL ist die kurze, menschenlesbare Reproducer-Seite für Forum und Support. Sie wird
vom vorhandenen Spring-Boot-Prozess ausgeliefert und verlinkt den öffentlichen RegressionGPT,
das exakte OpenAPI-Dokument, die GPT-Instructions und den Quellcode.
...
- `TICKET_TEMPLATE.md`: englische Supportvorlage für den Cross-Turn-Befund
...
- [Dateien als dokumentierte Conversation-Artefakte zurückgeben](https://developers.openai.com/api/docs/actions/sending-files)
