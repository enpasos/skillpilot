...
import { germanContract } from "../server/contracts/de.mjs";
...
test("opaque widget references stay private while the learning state survives reload", async () => {
  ...
  const publicState = publicCoachState(opened, germanContract);
  const privateMeta = privateWidgetMeta(opened, germanContract);
  ...
  const pending = await store.pending(germanContract);
  assert.match(pending.submission.answer, /x = 7/);

  await store.evaluate(germanContract, {
    score: 2,
    maxScore: 2,
    passed: true,
  ...
  const persisted = await reloaded.current(germanContract);
