...
Implementierungsstand: Paketmaterialisierung und unabhängige Finished-ZIP-Prüfung aus DPK-005b, der read-only Store-/Snapshot-Consumer aus DPK-006a, die sichere content-adressierte Provisionierung samt CAS-Aktivierung/Rollback aus DPK-006b sowie die vollständige Package-Service-Umschaltung DPK-006c-a–f sind technisch umgesetzt. Der reale Mathematik-Conformance-Lauf führt Build, Validator, Installation, Reverify, Aktivierung und die echten Java-Domain-/View-/Resource-/Source-Evidence-States mit 1.079 Zielen, 33.382 Source-Mappings, 88 Views/Offerings, 12 Decks/128 Karten, 756 Bildressourcen und 9.977 SourceGoals zusammen; Frontendtests binden dieselben öffentlichen Identitäten ohne Artefaktpfade oder eingebrannte Source-Indizes. DPK-007 bleibt offen: Erst der hermetische Package-only Smoke-Test darf für den gesamten gestarteten Anwendungsfluss belegen, dass die Software weder den Curriculum-Quellbaum noch fachliche Daten aus `app/public` oder `app/src/data` verwendet. Auch ein technisch valides Paket ist vor Abschluss der menschlichen Rechte- und Fachreviews kein veröffentlichbarer Release.
