...
| nur Backend-URLs wortgetreu; keine Links aus IDs oder mit Tokens; passendes Bild des aktiven atomaren Ziels als Orientierung im Cockpit und in der OpenAI-MCP-UI | globale Context-Policy, allowlist-projizierte Ressourcen, optionale `goalVisualization` und versionierte UI-Ressource | kurze Server-Invariante, Skill-Ausgaberegel und sichere Projektion | nur der dedizierte Renderer bindet die UI; nach einem freigebenden Vollresultat läuft er mit dessen Ziel-ID und `expectedStateVersion` unmittelbar und genau einmal; der Renderer validiert Backendzustand und Ziel erneut; private oder nicht passende Bildpfade und interne Identität werden entfernt; ohne gültiges Bild bleibt der normale Chat |
