# Landscape JSON Field Reference (DE_HES_S_GYM_2_MATHEMATIK.de.json)

Scope
- Source: `curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_MATHEMATIK.de.json`
- Goal count (current file): 356
- This describes the fields that exist in the JSON and how the app currently uses them.
...
- Used by: `app/src/hooks/useLandscapes.ts` (API path and landscape selection)
- Status: used

locale
- Type: string (BCP-47 locale tag)
- Example: "de-DE"
- Used by: not referenced in app code; language selection is handled by loading a locale-specific JSON file
...
- Status: used for UI lists and selection
...
- Used by: not defined in `app/src/landscapeTypes.ts`, so it is ignored by the typed loader
- Status: currently unused in UI
...
- Used by: not used in UI; `app/src/hooks/useAppCore.ts` builds short keys from the UUID instead
...
- Type: string[]
- Example: ["GK", "LK"]
- Used by: filter logic in `app/src/components/CompetenceTree.tsx` and `app/src/views/LearnerView.tsx`; also used for tag display and SRS tag filtering
- Status: used
- Note: tags are generic; some other landscapes use tags like "srs-deck:*" or "select:*"
...
- Status: used
- Note: cross-landscape references of the form "<landscapeId>:<goalId>" are normalized in `app/src/hooks/useLandscapes.ts`
...
- Used by: `app/src/goalTypes.ts` to derive UI fields (phase, themenfeld, level, leitideen, kompetenzen)
...
- Status: currently unused in UI, but available for future filtering and reporting

Data quality notes (current file)
- 22 goals have no sourceRef
- 24 goals have empty processCompetencies and guidingIdeas
- 4 goals have missing demandLevel
- 22 goals have missing topicCode

Further analysis ideas
- Create a JSON Schema file and validate all curricula against it
- Add a graph lint step (missing references, cycles, orphan nodes)
- Normalize or document tag vocabularies (GK/LK, srs-deck, select)
- Surface dimensionTags (phase/area/level) in UI filters or reports
- Reconcile shortKey usage (JSON shortKey vs slug-from-id in UI)
