...
The Builder CLI is implemented in [buildFullStandaloneCurriculumPackage.ts](../../app/scripts/buildFullStandaloneCurriculumPackage.ts); finished archives are checked by the implementation-independent [full package validator](../../scripts/validate_full_standalone_curriculum_package.py). The evaluator invokes that validator independently again and accepts its report only when validator identity, archive hash, package identity, complete gate set, evidence and process exit agree.
