...
That result still deliberately reports `conformance-model-only-not-a-package`. DPK-004b must classify mapping, source, and quality publication evidence. DPK-005 must then copy all required assets and wrap the completed model with a trusted manifest, offline schema catalog, complete inventory, license resolution, checksums, and finished-ZIP safety validation. A later package-only consumer smoke test must prove that SkillPilot uses no curriculum source tree or `app/public` fallback. Until those gates pass, `ready` remains unavailable and existing Subject Export ZIPs remain `not-ready-legacy`. The current program checkpoint and remaining sequence are maintained in [Dual Curriculum Package Implementation Status](../dev/dual-curriculum-package-implementation-status.md).
