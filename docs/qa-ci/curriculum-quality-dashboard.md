...
| `M7` | `M6` plus complete strict D/P/A/M/V coverage of every current `curricularAtomic` goal, successful required validation checks and no relevant blockers. |

`M5` is intentionally strict for the core curriculum layer. `M6` is stricter again and is reserved for curricula whose optional memory-card layer has also been semantically reviewed. `M7` adds full deep curriculum QA coverage; it does not weaken any lower curriculum or memory-layer rule or grant human approval. Human trial status is a separate statement, as defined in the [quality and human-trial concept](../concept/curriculum-quality-and-human-trial.md).
...
The current rule catalog is versioned as `curriculum-quality-v5`.
...
| `CQR-303` | `M7` | The central report from `app/scripts/reportDeepUnderstandingRollout.ts` proves the exact nonempty current `curricularAtomic` scope and intersection of description reviews, positive evidence, atomicity, memory and visualization gates. All required checks pass and no blocking findings remain. Technical image deferrals never count as completed. Gate V requires a current exact hash-bound allowed AI review; `humanApproved` and `humanIssueIdentified` neither satisfy nor veto V or M7. Mandatory human release and trial gates remain separate. |
