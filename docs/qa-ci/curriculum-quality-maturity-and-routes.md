...
- V verlangt eine aktuelle fachliche AI-Bildentscheidung, die an den exakten aktiven Asset-Hash sowie gueltige Ziel-/Text- und Public-/Canonical-Asset-Bindungen gebunden ist. Das Gate ist maschinell: `humanApproved` und `humanIssueIdentified` erfuellen oder blockieren V und M7 nicht; menschliche Release- und Erprobungsbefunde werden getrennt ausgewiesen.
...
- `deferred_provider_limitation` und `deferred_quality_review` sind dokumentierte, weiterhin offene Bildarbeit und erfuellen V nicht. Die zweite Entscheidung setzt den nachgewiesenen Hash des fachlich beanstandeten, archivierten Originalbilds und einen konkreten Befund voraus; sie behauptet keine Providergrenze. Im Bildmodell gibt es keine fachliche Ausnahme, die aus einer Rueckstellung einen Abschluss macht.
