# Canonical Gymnasium Chemistry Schleswig-Holstein Cutover Readiness Audit

Date: 2026-05-12

## Decision

`DE-SH` Chemistry is cutover-ready on the canonical Gymnasium Chemistry route.

The operational learner route is the shared `Gymnasium (DE)` root with the canonical Chemistry child landscape and the matching `DE-SH` GK/LK composition view. The retained Schleswig-Holstein source-extraction landscapes remain archive-only evidence surfaces.
...
- Reviewed source-to-canonical mappings are clean and now have runtime mapping files:
  - `curricula/DE/Gymnasium/mapping/DE-SH/lower-secondary/sh_chemistry_lower_secondary_to_canonical_chemistry.json`
  - `curricula/DE/Gymnasium/mapping/DE-SH/upper-secondary/sh_chemistry_upper_secondary_to_canonical_chemistry.json`
- The canonical learner-facing views exist for both course profiles:
  - `de-sh-gym-chemistry-gk`
  - `de-sh-gym-chemistry-lk`
...
Schleswig-Holstein does not currently expose an active committed legacy Chemistry learner route comparable to the Hessen or Bayern legacy trees. The required runtime cutover check is therefore the retained-source fence plus runtime mapping discovery:

- retained source landscapes load from the real source registry
- retained source landscapes resolve to `DE-SH`
