...
1. `J8 Raum und Form`
   - quadrilaterals, triangles, congruence
   - transformations, symmetry, similarity
   - Pythagoras
   - Thales and angle arguments

2. `J10 Raum und Form`
   - properties, positional relations, similarity
...
3. later subcorridors
   - `Vierecke, Dreiecke und Kongruenzsaetze (Sek I)`
   - `Kreisbeziehungen und Satz des Thales (Sek I)`
   - `Spaetere Flaechen-, Kreis-, Koerper- und Aehnlichkeitsvorstellungen (Sek I)`

4. important later atoms already present
   - `Kongruenz begruenden und Dreieckskonstruktionen ausfuehren`
   - `Dreieckskonstruktionen nach Kongruenzsaetzen planen, ausfuehren und ihre Loesbarkeit begruenden`
...
- the current canonical geometry layer mixes
  ...
  - subtopic structure,
  - and some repeated bridge atoms
  in a way that makes state evidence harder to compare topic by topic

This is not a missing-content problem first.
It is a packaging and cut-quality problem.
...
2. `G2 Triangles, quadrilaterals, congruence, constructions`
   - figure properties
   - congruence
...
4. `G4 Circle relations and Thales`
   - circle relations
   - Thales
   - angle reasoning around circles

5. `G5 Pythagoras and geometric justification`
   - right triangles
   - Pythagoras in calculation and construction

6. `G6 Bodies, area, volume, and later space problems`
   - prisms, cylinders, pyramids, cones, spheres
   - surface / volume
   - later space modelling

7. `G7 Trigonometric bridge`
   - only where truly Sek-I-wide and shared
...
2. add explicit canonical subclusters for
   - `G3 Transformations, symmetry, similarity`
   - `G5 Pythagoras and geometric justification`
3. repackage the later Sek-I geometry corridor so that figure/congruence work, transformation/similarity work, circle/Thales work, and Pythagoras work are visible as explicit subpackages
4. keep first body-geometry content attached as a compatibility bridge for now instead of forcing a second large cut in the same step
