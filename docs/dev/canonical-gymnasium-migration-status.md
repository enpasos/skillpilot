...
- Bavaria Gymnasium adopted mapping files present: `5`
- `npm run validate:view-filters` currently returns `0` errors, `0` active warnings, `37` accepted warnings
...
- `validate:view-filters` is now clean on active reviewed findings for that scope: `0` errors, `0` active warnings, `37` accepted warnings recorded in `docs/qa-ci/applicability-accepted-warnings.json`
...
| `curricula/DE/BY/Gymnasium` | partial | done | done | done | done | done | 90.0% | no | Bavaria now has canonical-by-default entry plus compatibility-only retirement across the live legacy tree, but canonical replacement breadth still covers only a five-subject adopted corridor inside a `45`-subject source tree | adopt at least one additional Bavaria subject corridor beyond the current Math/Physics/Chemistry/Biology/Informatik lane and then execute the final delete handoff |
