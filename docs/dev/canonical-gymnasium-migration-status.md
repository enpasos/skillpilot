...
- Bavaria Gymnasium adopted mapping files present: `6`
- `npm run validate:view-filters` currently returns `0` errors, `0` active warnings, `45` accepted warnings
...
| `curricula/DE/BY/Gymnasium` | partial | done | done | done | done | done | 90.0% | no | Bavaria now has canonical-by-default entry plus compatibility-only retirement across the live legacy tree, but canonical replacement breadth still covers only a six-subject adopted corridor inside a `45`-subject source tree | adopt at least one additional Bavaria subject corridor beyond the current Math/Physics/Chemistry/Biology/Informatik/Wirtschaft_und_Recht lane and then execute the final delete handoff |
