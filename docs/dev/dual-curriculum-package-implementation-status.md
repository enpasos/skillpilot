...
- reale Store-Probe: alle 88 View-Payloads des 1,7-GB-Mathematikpakets laden; jedes Single-Offering ist per ID und exakt gleichem Scope identisch auflösbar, während der Compiler für exakt gepaarte GK-/LK-Views ein deklariertes, dedupliziertes `GK+LK`-Composite-Offering erzeugt; freie Merge-IDs bleiben unaufgelöst;
- vollständiges `./run_ci.sh` auf dem final dokumentierten Stand grün.
