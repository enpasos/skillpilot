...
- `DE-NW` (`P5`, `active`): Broaden Nordrhein-Westfalen from its now multi-corridor lower-secondary prerequisite/functions and upper-secondary analysis, geometry/linear-algebra, five explicit stochastic pilot strips, the refined GK Hauptsatz/Stammfunktions split, the now exact GK/LK stock-effect surface, the now exact retained exponential split, the now exact E-phase line/segment parameter-form leaf, the now exact GK scalar-product leaf, the now exact GK coordinate-form-orientation leaf, the now exact GK line-plane-intersection leaf, and two explicit LK integral strips by narrowing next to another equally explicit geometry leaf such as the intersection-angle leaf, or returning to the retained E-phase power/polynomial split, rather than reopening parent cleanup inside the current NRW snapshots.
