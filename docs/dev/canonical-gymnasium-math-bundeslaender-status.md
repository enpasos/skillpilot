...
- `DE-NW` (`P5`, `active`): Broaden Nordrhein-Westfalen from its now multi-corridor lower-secondary prerequisite/functions and upper-secondary analysis, geometry/linear-algebra, five explicit stochastic pilot strips, the refined GK Hauptsatz/Stammfunktions split, the now exact LK Hauptsatz-with-intuitive-continuity leaf, the now exact GK/LK stock-effect surface, the now exact transition-to-integral leaf, the now exact LK improper-integral-area leaf, the now exact LK rotation-volume leaf, the now exact retained intro power/polynomial split, the now exact retained exponential split, the now exact E-phase line/segment parameter-form leaf, the now exact E-phase line-relation leaf, the now exact explicit LK line-/plane-relation leaf, the now exact GK scalar-product leaf, the now exact GK coordinate-form-orientation leaf, the now exact GK line-plane-intersection leaf, the now exact GK intersection-angle leaf, the now exact LK approximation-in-situations leaf, and the now exact LK mu/sigma-on-normal-distribution leaf by moving next to another equally explicit imported leaf outside the exhausted LK integral strip, such as the retained LK normal-distribution-computation residue around diskrete/stetige Zufallsgroessen und Verteilungsfunktion, rather than reopening broader parent cleanup inside the current NRW snapshots.
