...
1. broaden Nordrhein-Westfalen from its now atom-anchored lower-secondary prerequisite/functions plus upper-secondary analysis and first E-phase/Q-phase geometry/linear-algebra pilot coverage toward broader state coverage, but do so via the remaining explicit NRW Q-phase geometry residue or another equally explicit NRW source corridor rather than reopening structural parent cleanup inside the already reviewed NRW snapshots
