...
1. treat the shared `DE-BB` / `DE-BE` upper-secondary Q3 geometry strip as exhausted, keep Berlin's now parent-anchored matrix / transition, sequences / series, differential-equations, complex-numbers, logic, and reasoning / proof side lanes as the established BE-local counterweight, and keep Brandenburg's linear-representation / projection side lane as the first BB-local counterweight
2. decide next whether Brandenburg can widen this tuple / point-vector, spreadsheet, and projection entry into a fuller matrix / linear-model strip or whether Berlin deepens one of the now thinner remaining local optional-course lanes such as analysis-deepening or numerical-mathematics while alignment remains explicit
