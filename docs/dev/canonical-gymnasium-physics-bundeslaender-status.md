...
- `DE-NW`: Either keep widening the NRW upper-secondary lane through the last still-missing reviewed atom-model residue around Spektralanalyse/Modellentwicklung, or stop local widening and move to the next state lane; do not invent NRW-specific Physics composition views before broader reviewed evidence actually requires them.
