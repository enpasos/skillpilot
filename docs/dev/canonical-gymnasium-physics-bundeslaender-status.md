...
| `DE-NI` Niedersachsen | `P4` First corridor reviewed | `65%` | `yes` | `12` | `subtree_adopted` | `active` |
...
- `DE-NI` Niedersachsen: Niedersachsen now has an active upper-secondary Physics source snapshot, provenance-backed membership and closure, and a reviewed narrow mapping lane on one shared orientation anchor, one E-phase `Dynamik` corridor, and one adjacent E-phase energy strip. The lane currently carries 12 reviewed mappings and a committed applicability cut on the shared free-fall, horizontal-throw, Newton, circle-motion, kinetic-energy, and energy-conservation strip, while the sustainability-related Bewertungsblatt from the same KC pages remains intentionally source-led. The lane still avoids Niedersachsen-specific Physics composition views and new canonical Physics atoms.
...
- `DE-NI`: Keep the Niedersachsen upper-secondary Physics lane narrow and reviewed: the next clean widening should now move to the first equally explicit qualification-phase anchor such as `Elektrizitaet` or `Schwingungen und Wellen`, unless the remaining sustainability-only Bewertungsblatt inside `Dynamik` is intentionally resolved first; avoid Niedersachsen-specific Physics composition views or new canonical atoms without wider reviewed evidence.
