...
| `DE-NW` Nordrhein-Westfalen | `P4` First corridor reviewed | `65%` | `yes` | `28` | `subtree_adopted` | `active` |
...
- `DE-NW`: Move to the next state lane, or at most choose another equally explicit NRW corridor outside the now exhausted GK atom-model strip; do not invent NRW-specific Physics composition views before broader reviewed evidence actually requires them.
