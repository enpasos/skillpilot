...
| `SEK2.NI.UPPER_SECONDARY_ENTRY` Niedersachsen upper-secondary first entry lane | `active` | `DE-NI` | Keep the Niedersachsen upper-secondary Physics lane narrow and reviewed: after the now exhausted Q1 electricity page-36 route, the next clean widening should open `Schwingungen und Wellen`; revisit the still source-led `Querfeld`/`Wien-Filter` or `Fadenstrahlrohr` residues only if broader reviewed shared targets become justified; keep avoiding Niedersachsen-specific Physics composition views or new canonical atoms without wider reviewed evidence. |
...
| `DE-NI` Niedersachsen | `P4` First corridor reviewed | `65%` | `yes` | `44` | `subtree_adopted` | `active` |
...
- `DE-NI` Niedersachsen: Niedersachsen now has an active upper-secondary Physics source snapshot, provenance-backed membership and closure, and a reviewed narrow mapping lane on one shared orientation anchor, one E-phase `Dynamik` corridor, one adjacent E-phase energy strip, one first qualification-phase `Elektrizitaet` strip on the shared Q1 potential-and-capacitor surfaces, one adjacent reviewed magnet-field strip, one adjacent reviewed induction strip, one adjacent reviewed LK follow-on on the same page-36 route, and now also one first reviewed Q2 oscillation strip on the shared mechanical and electromagnetic oscillation surface. The lane currently carries 44 reviewed mappings and committed applicability cuts on the shared free-fall, horizontal-throw, Newton, circle-motion, kinetic-energy, energy-conservation, current/voltage, capacitor, magnetic-field, induction, Hall-Effekt, differential induction, self-induction, coil-field-energy, harmonic oscillation, characteristic oscillation quantities, restoring-force, damping, resonance, electromagnetic oscillating-circuit, and Thomson strip, while the sustainability-related Bewertungsblatt from the same E-phase KC pages plus the crossed-field / `Wien-Filter` and `Fadenstrahlrohr` residues remain intentionally source-led. The lane still avoids Niedersachsen-specific Physics composition views and new canonical Physics atoms.
...
- `DE-NI`: Keep the Niedersachsen upper-secondary Physics lane narrow and reviewed: after the first Q2 oscillation strip, the next clean widening should open `Wellen`; revisit the still source-led `Querfeld`/`Wien-Filter` or `Fadenstrahlrohr` residues only if broader reviewed shared targets become justified; avoid Niedersachsen-specific Physics composition views or new canonical atoms without wider reviewed evidence.
