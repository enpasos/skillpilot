...
- the former RP oscillation/wave micro-residues on `Periodendauer`, `Polarisation`, `Weisslichtspektrum`, and `gekoppelte Schwingungen` now land conservatively on shared canonical oscillation and interference surfaces
- the former RP quantum / atom-model application residues on Planck estimation, `Lumineszenz`, `chemische Bindung/Molekuelstruktur`, `Farbstoffmolekuele`, `Paarerzeugung`, and `Offene Fragen der Elementarteilchenphysik` now land conservatively on shared canonical method, atom-model, antimatter, and research surfaces
- the former RP relativity / astrophysics / cosmology / chaos / fluid-dynamics residues now also land conservatively on shared canonical surfaces, with the shared canonical additions `Masse-Energie-Äquivalenz einordnen`, `Strömungsphysik`, and `Kontinuitätsgleichung, Strömungsgesetze und Reynolds-Zahl einordnen` closing the remaining genuine floor gaps
- next step: treat the RP lane as explicitly semantically closed on the current reviewed cut and reopen it only if a later source revision changes visible scope or another later retained source lane creates a genuinely shared Physics gap
