...
`B(0)=1200`, `B(3)=1200*0.82^3≈661.7 Wh`.

`1200*0.82^t<500` ergibt `t>log(500/1200)/log(0.82)≈4.41`. Die Halbwertszeit erfüllt `0.82^t=0.5`, also `t=log(0.5)/log(0.82)≈3.49`. Logarithmen sind nötig, weil die gesuchte Zeit im Exponenten steht.

`K(5)=2500*1.035^5≈2969.22`. Anfangskapital `2500 EUR`, Zinssatz `3.5 %`, Laufzeit `5` Jahre, Endkapital etwa `2969.22 EUR`; Zinseszins bedeutet, dass Zinsen in Folgejahren mitverzinst werden.
...
Ein passender Term ist `q(t)=6+2*cos((pi/6)*(t-3))`, weil `2pi/(pi/6)=12` und bei `t=3` ein Maximum liegt.

`150 Grad = 5pi/6`.

Aus `sin(alpha)=0.6` folgt `cos(alpha)=0.8`, da der Winkel spitz ist. `tan(alpha)=0.6/0.8=0.75`; `sin(90 Grad-alpha)=cos(alpha)=0.8`.

`H'(t)=14*(pi/20)*cos((pi/20)*(t-5))`. Bei `t=15` ist der Sensor im Maximum; `H'(15)=0`, die momentane Höhenänderung ist dort null.
...
`f` ist kubisch mit positivem Leitkoeffizienten; für `x->infty` gilt `f(x)->infty`, für `x->-infty` gilt `f(x)->-infty`. `f'(x)=0.3x^2-1.2x+0.9=0.3(x-1)(x-3)`, `f''(x)=0.6x-1.2`.
...
Bei `sqrt(x+5)=x-1` muss `x>=1` gelten. Quadrieren liefert `x+5=x^2-2x+1`, also `x^2-3x-4=0` und `x=4` oder `x=-1`; durch Definitionsbedingung und Einsetzen bleibt nur `x=4`.

Aus `x^4=81` folgen die reellen Lösungen `x=3` und `x=-3`.

Setze `z=x^2`; dann `z^2-5z+4=0`, also `z=1` oder `z=4`. Rücksubstitution ergibt `x=+-1` oder `x=+-2`.
...
`V=(1/3)*pi*r^2*h=(1/3)*pi*2.4^2*6=11.52pi≈36.2 m^3`. Die Formel ist plausibel, weil ein Kegel als Grenzfall von Pyramiden mit immer mehr Ecken in der Grundfläche verstanden werden kann.
