...
- source-landscape duration-model metadata (`G8`/`G9`) where a source curriculum is tied to a Gymnasium duration model and later learner-facing projections must distinguish that lane
...
- extraction/onboarding pipelines should persist `durationModel`/`durationModels` on source-extraction metadata and source-landscape registry entries whenever the official Lehrplan is explicitly G8 or G9; do not encode that distinction by duplicating canonical goals
