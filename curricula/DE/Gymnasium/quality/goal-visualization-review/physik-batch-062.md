...
- The first candidates for mass/weight force, force/extension, laser safety, and electric motor were rejected or not used because they contained avoidable arrow, label, beam-path, or language issues. Targeted prompt appends were tightened and the accepted images came from the first regeneration.
...
| `45bbdf6b-6372-5b6a-b7e4-be15a0eb4b83` | Kraft und Verformung experimentell untersuchen | `accepted_pilot_after_regeneration` | The first candidate was rejected because the middle spring state was also labeled `F = 0 N` despite showing the `1 N` load. The accepted regeneration shows three increasing spring extensions, the table rows `(0 N, 0 cm)`, `(1 N, 2 cm)`, `(2 N, 4 cm)`, and a rising straight graph through `(0|0)`, `(1|2)`, and `(2|4)`. Downward force arrows are used only for loaded states. |
...
- `4` first candidates were rejected before final import and regenerated.
