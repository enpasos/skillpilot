...
| `1ce8af38-082a-477b-af48-b924c92761bf` | Ganzrationale Funktionen über Term und Graph beschreiben | `accepted_pilot` | The example `f(x)=x^3-3x` is matched with a plausible cubic graph, zeros at `-sqrt(3)`, `0`, and about `1.73`, extrema near `x=-1` and `x=1`, and correct left-down/right-up end behavior. The caution that a graph provides clues but the term must be checked is appropriate. |
...
- No Batch 021 asset is marked `deferred_provider_limitation`.
