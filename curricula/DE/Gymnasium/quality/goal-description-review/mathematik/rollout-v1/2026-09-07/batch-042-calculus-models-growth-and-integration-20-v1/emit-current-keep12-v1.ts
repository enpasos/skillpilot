...
const prefix = 'curricula/DE/Gymnasium/quality/goal-description-review/mathematik/rollout-v1/2026-09-07/batch-042-calculus-models-growth-and-integration-20-v1'
const helperPath = 'curricula/DE/Gymnasium/quality/goal-description-review/mathematik/rollout-v1/2026-09-07/batch-042-calculus-models-growth-and-integration-20-v1/emit-current-keep12-v1.ts'
const expectedGoalIds = ["dc12f281-f161-572b-a973-8405ae9b2498","0b162cb0-8507-5ac2-b9d6-57f40f4d3f35","71fe4a39-38e8-5c6a-8eef-ff4783fe70c2","93fcf49d-a08c-5aa5-a5b2-cb4ee56fdae4","4ae9e316-509f-517d-bd94-a165817af24f","886caebc-a042-4a94-91f9-6dc184203c42","e7350739-c89f-5c7b-b4d1-717d6a767298","164921f6-3bf7-5efc-a438-ea4759dca9ef","b71c332f-ef9d-5c27-983b-7103269ff419","dc3c6e19-8ac2-532c-8d64-ca6e3d3f5cd9","d108ec1f-2c8f-5267-be75-10e1b77edd7a","6d89d813-29dc-5e76-b004-98ed6b6fe8ce"]
const explicitHolds: Record<string, string> = {
  "281e0c6e-d06d-580f-8443-4369f0ea524b": "KEEP/BLOCK: disk illustration versus horizontal strips in the radius diagram requires substantive visual adjudication; current pair does not close.",
  "f4935b24-d8a9-5eb7-a5eb-6d34a9e09b2d": "REVISE/KEEP: signed improper-integral convergence versus geometric area and singularity wording requires substantive bilingual adjudication; no silent override.",
  "2713980f-75d2-5455-a8cb-bcd3888c49a0": "BLOCK/BLOCK: wrong maximum abscissa and finite spurious zero in the bound graph; final corrected image must be actually re-reviewed.",
  "e9ad45b9-c0d2-5804-b6bf-79e5ce041d2c": "BLOCK/BLOCK: English title rational functions conflicts with polynomial scope; narrowly correct canonical English title then re-review affected final context.",
  "ccd83f05-a0ae-4cd1-8917-24793a219fee": "BLOCK/BLOCK: intermediate plotted values contradict H(t)=80−60 exp(−0.4t); quantitative image correction required.",
  "848af536-c7e5-4df0-a4e9-d5d0ff15244c": "BLOCK/BLOCK: graph reverses the time order of E=20 and logistic inflection; correct shared plot.",
  "df7338ef-65ba-5ece-8aec-7f520dfe5710": "BLOCK/BLOCK: bounded exponential growth does not have an initial inflection point; remove the false exception from the image.",
  "5ccf0d90-2967-576e-8f72-f002792d8515": "BLOCK/BLOCK: substitution bubble equates 2x with du instead of du=2x dx; final corrected raster required."
}
// Read-only patch emitter for the explicitly selected current B042 KEEP/KEEP pairs after full A/B record-body synthesis.
...
async function main() {
  ...
  const authored = json(prefix + '/synthesis-authoring.current-keep12-v1.json')
  ...
  assert.equal(existsSync(prefix + '/current-keep12-v1.currency-receipt.json'), checkMode, 'Use --check for existing artifacts; never replay writes')
  const sourceBytesBefore = new Map<string, Buffer>([prefix + '.config.json', prefix + '/batch-manifest.json', prefix + '/dual-summary.json', prefix + '/synthesis-authoring.current-keep12-v1.json', helperPath].map(path => [path, read(path)]))
  ...
  const currentStateAudit = config.goalIds.map((id: string) => {
    ...
    const mixedKeepRevise = false // This B042 subset admits only the twelve explicitly reviewed KEEP/KEEP pairs.
  ...
  const manifestPath = 'synthesis-decisions.current-keep12-v1.json'
  ...
  for (const [i, goal] of expectedGoals.entries()) {
    ...
    const path = `resolutions-current-keep12-v1/${goal.goalId}.resolution.json`
  ...
  console.log(JSON.stringify({ patch, summary: { selected: entries.map(e => e.goalId), files: files.length, indexPath: prefix + '/resolution-index.current-keep12-v1.json' } }))
}
void main()
