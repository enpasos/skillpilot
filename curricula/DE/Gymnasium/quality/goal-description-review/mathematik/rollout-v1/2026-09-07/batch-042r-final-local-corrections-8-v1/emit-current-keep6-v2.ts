...
const prefix = 'curricula/DE/Gymnasium/quality/goal-description-review/mathematik/rollout-v1/2026-09-07/batch-042r-final-local-corrections-8-v1'
const helperPath = 'curricula/DE/Gymnasium/quality/goal-description-review/mathematik/rollout-v1/2026-09-07/batch-042r-final-local-corrections-8-v1/emit-current-keep6-v2.ts'
const expectedGoalIds = ["281e0c6e-d06d-580f-8443-4369f0ea524b","2713980f-75d2-5455-a8cb-bcd3888c49a0","ccd83f05-a0ae-4cd1-8917-24793a219fee","848af536-c7e5-4df0-a4e9-d5d0ff15244c","df7338ef-65ba-5ece-8aec-7f520dfe5710","5ccf0d90-2967-576e-8f72-f002792d8515"]
const explicitHolds: Record<string, string> = {
  "f4935b24-d8a9-5eb7-a5eb-6d34a9e09b2d": "BLOCK/KEEP after fresh current review: retained 1/sqrt(x) raster has quantitatively wrong curve at x=.25 and .5 under its marked unit scales. Correct wording and correct integral value do not resolve the visible graph error; preserve dissent for substantive adjudication.",
  "e9ad45b9-c0d2-5804-b6bf-79e5ce041d2c": "BLOCK/KEEP after fresh current review: retained x²exp(-x) raster has right tail at x4 around22% rather than54% of maximum under its marked linear scales. Correct polynomial EN title and derivative formulas do not repair that graph; preserve dissent."
}
...
async function main() {
  ...
  const authored = json(prefix + '/synthesis-authoring.current-keep6-v2.json')
  const config = json(prefix + '.config.json')
  assert.equal(config.goalIds.length, 8)
  assert.deepEqual(authored.decisions.map((d: any) => d.goalId), expectedGoalIds)
  assert.equal(existsSync(prefix + '/current-keep6-v2.currency-receipt.json'), checkMode, 'Use --check for existing artifacts; never replay writes')
  const sourceBytesBefore = new Map<string, Buffer>([prefix + '.config.json', prefix + '/batch-manifest.json', prefix + '/dual-summary.json', prefix + '/synthesis-authoring.current-keep6-v2.json', helperPath].map(path => [path, read(path)]))
  ...
  const currentStateAudit = config.goalIds.map((id: string) => {
    ...
    const mixedKeepRevise = false // This B042r subset admits only the six explicitly reviewed KEEP/KEEP pairs.
  ...
  const manifestPath = 'synthesis-decisions.current-keep6-v2.json'
  ...
  for (const [i, goal] of expectedGoals.entries()) {
    ...
    const path = `resolutions-current-keep6-v2/${goal.goalId}.resolution.json`
  ...
  console.log(JSON.stringify({ patch, summary: { selected: entries.map(e => e.goalId), files: files.length, indexPath: prefix + '/resolution-index.current-keep6-v2.json' } }))
}
void main()
