...
const records = input.goals.map((g, index) => {
  ...
  assert(['keep', 'block', 'revise', 'split_review'].includes(a.decision));
  if (a.decision === 'revise') assert(a.proposedDescriptionDe && a.proposedDescriptionEn, 'Only explicitly authored bilingual proposals');
  else assert(!a.proposedDescriptionDe && !a.proposedDescriptionEn);
  ...
  return {
    ...
    decision: a.decision, ...(a.decision === 'revise' ? { proposedDescriptionDe: a.proposedDescriptionDe, proposedDescriptionEn: a.proposedDescriptionEn } : {}), understandingEvidence: a.understandingEvidence, rationale: a.rationale,
...
const provenance = {
  ...
  decisions: { keep: records.filter((r) => r.decision === 'keep').length, block: records.filter((r) => r.decision === 'block').length, revise: records.filter((r) => r.decision === 'revise').length, split_review: records.filter((r) => r.decision === 'split_review').length },
