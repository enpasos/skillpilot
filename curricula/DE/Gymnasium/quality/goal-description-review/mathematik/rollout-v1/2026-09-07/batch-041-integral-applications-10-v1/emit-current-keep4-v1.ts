...
const prefix = 'curricula/DE/Gymnasium/quality/goal-description-review/mathematik/rollout-v1/2026-09-07/batch-041-integral-applications-10-v1'
const helperPath = 'curricula/DE/Gymnasium/quality/goal-description-review/mathematik/rollout-v1/2026-09-07/batch-041-integral-applications-10-v1/emit-current-keep4-v1.ts'
const expectedGoalIds = ["649b673c-1a74-5dc5-af01-b4c9e090b90d","e9114fc2-1a87-5ef5-8fa3-7ee4c9bbe0dd","647888d2-8e23-52b2-a2ec-fad3f31cb1a6","0f180645-37ce-5b6b-8a36-ad7b31168b1a"]
const explicitHolds: Record<string, string> = {
  "31be24f0-3ab1-54d2-856d-fa9b7f36552f": "SPLIT/SPLIT: constructing polynomial primitives and using supplied primitives require a scoped consolidation decision; no closure by text equality.",
  "809ef78a-f282-5593-89be-0f2cb95570ac": "SPLIT/SPLIT: reconstructing stocks, mean stocks and mean rates are independently assessable; inspect existing goals before splitting.",
  "34604a97-0c64-5b06-81e2-6ac818732d60": "KEEP/BLOCK: bound image describes unweighted accumulated height rather than width-weighted contributions. Requires actual image/content correction and current review.",
  "23c8b5f9-ab35-4071-a3d4-b76a669a0995": "REVISE/REVISE: stock reconstruction omits the required initial value; canonical revision must be coordinated before closure.",
  "0d21097c-09bf-5375-8c56-34ce8dc5bc35": "KEEP/REVISE: domain restrictions and role of the inner derivative need explicit fachliche adjudication, not a majority vote.",
  "19481f5d-94de-4a74-b765-cbebd1525994": "KEEP/REVISE: German root functions versus narrower English square-root functions need a scoped bilingual parity decision."
}
// Read-only patch emitter for the explicitly selected current B041 KEEP/KEEP pairs after full A/B record-body synthesis.
...
async function main() {
  ...
  const authored = json(prefix + '/synthesis-authoring.current-keep4-v1.json')
  const config = json(prefix + '.config.json')
  assert.equal(config.goalIds.length, 10)
  assert.deepEqual(authored.decisions.map((d: any) => d.goalId), expectedGoalIds)
  assert.equal(existsSync(prefix + '/current-keep4-v1.currency-receipt.json'), checkMode, 'Use --check for existing artifacts; never replay writes')
  const sourceBytesBefore = new Map<string, Buffer>([prefix + '.config.json', prefix + '/batch-manifest.json', prefix + '/dual-summary.json', prefix + '/synthesis-authoring.current-keep4-v1.json', helperPath].map(path => [path, read(path)]))
  ...
  const currentStateAudit = config.goalIds.map((id: string) => {
    ...
    const mixedKeepRevise = false // This B041 subset admits only the four explicitly reviewed KEEP/KEEP pairs.
  ...
  const manifestPath = 'synthesis-decisions.current-keep4-v1.json'
  ...
  for (const [i, goal] of expectedGoals.entries()) {
    ...
    const path = `resolutions-current-keep4-v1/${goal.goalId}.resolution.json`
  ...
  console.log(JSON.stringify({ patch, summary: { selected: entries.map(e => e.goalId), files: files.length, indexPath: prefix + '/resolution-index.current-keep4-v1.json' } }))
}
void main()
