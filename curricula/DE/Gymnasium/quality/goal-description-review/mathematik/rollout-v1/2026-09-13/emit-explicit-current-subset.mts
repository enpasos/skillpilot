// Adapted from the retained D048 emitter for explicit current Mathematics subsets.
...
const kinds = JSON.parse(fs.readFileSync('curricula/DE/Gymnasium/quality/release-model/mathematik.semantic-kinds.json', 'utf8'))
...
for (const decision of spec.decisions) {
  ...
  const resolutionDecision = decision.resolutionDecision ?? 'keep_current'
  if (!['keep_current', 'current_after_revision'].includes(resolutionDecision)) throw Error('Unsupported explicit resolution decision: ' + decision.goalId)
  ...
  const resolution = buildGoalDescriptionDualRoundResolution({
    ...
    goalId: decision.goalId, effectiveSemanticKind: 'curricularAtomic', decision: resolutionDecision,
    synthesis: {
...
const index = {
  ...
  subject: 'Mathematik', semanticKind: 'curricularAtomic', strictDescriptionReviewCompleteCount: resolutions.length,
