...
## Current one-goal image and P-v2 context

- Goal `34200b88-c616-58f6-aa03-efb9fd766f88`, Grade 7 perpendicular
  bisector. Read the actual current goal, normative source context, page, and
  bound image. The image shows only one equidistant point and tool cues; it is
  neither the whole line nor a compass-construction trace.
- Current P-v2 candidate:
  `curricula/DE/Gymnasium/quality/goal-evidence/m7-perpendicular-bisector-png-current-20260923-v1.review.jsonl`
  (`sha256:87b372e844ccb18c8f1082655d7294ecaac4c11993afde27deaa9c1ecb65fa17`).
  This candidate requires *both* a compass construction and a set-square
  construction, plus independent equal-distance locus reasoning, across three
  fresh cases. Its status is `needs_human_review` / `ai_candidate`, not a human
  approval or a learner achievement.
- Review the short DE/EN description independently from the P-v2 candidate;
  use the candidate only to judge whether positive evidence is already
  specified. A page illustration is not proof that the learner can construct.
