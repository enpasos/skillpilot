...
export async function buildViewCandidates({ root, beforeLandscape, afterLandscape, externalGoals = [] }: any) {
  ...
  assert(JSON.stringify(boundLeases) === JSON.stringify(leases), 'Affected view node/placement anchor lease changed; review its exact delta before rebinding')
  return { files, receipt: { checkedViewsIncludingAtlas: proof.length, changedStateViews: 64, changedNationalViews: 4, changedAtlasViews: 1, proof, leases } }
}
