// Separate output lane for the six remaining unchanged D048 KEEP/KEEP goals.
// Original emitter and the first five resolutions remain immutable.
...
if (dual.prepared.manifest.batchId !== 'physik-rollout-v1-batch-048-current-concepts-final-20-v1-20260908') throw Error('D048 only')
const authoringPath = join(dir, 'synthesis-remaining-six-v1.json')
...
for (const decision of spec.decisions) {
  ...
  const resolutionPath = 'resolutions-remaining-six-v1/' + decision.goalId + '.resolution.json'
...
const index = {
  schemaVersion: 1, artifactSetId: dual.prepared.manifest.batchId + '-explicit-remaining-six-v1',
