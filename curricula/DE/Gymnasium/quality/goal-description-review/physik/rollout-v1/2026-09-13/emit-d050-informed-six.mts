// Separate output lane for six individually adjudicated D050 KEEP/KEEP goals.
// Original campaigns, existing bb5 resolution and both source-route holds remain immutable.
...
if (dual.prepared.manifest.batchId !== 'physik-rollout-v1-batch-050-current-dependent-contexts-16-v1-20260908') throw Error('D050 only')
const authoringPath = join(dir, 'synthesis-informed-six-v1.json')
...
if (JSON.stringify(spec.decisions.map(x => x.goalId)) !== JSON.stringify(exactGoalIds)) throw Error('Exact six informed decisions required; source-route holds must remain excluded')
...
const resolutions = []
const receiptPath = join(dir, 'informed-six-materialization.receipt.json')
const checkedAt = checkOnly ? JSON.parse(fs.readFileSync(receiptPath, 'utf8')).checkedAt : new Date().toISOString()
for (const decision of spec.decisions) {
  ...
  const resolution = buildGoalDescriptionDualRoundResolution({
    resolutionId: dual.prepared.manifest.batchId + '-informed-six-' + decision.goalId,
    goalId: decision.goalId, effectiveSemanticKind: 'curricularAtomic', decision: 'keep_current',
    synthesis: {
      synthesisId: dual.prepared.manifest.batchId + '-informed-six-synthesis-' + decision.goalId,
  ...
  const resolutionPath = 'resolutions-informed-six-v1/' + decision.goalId + '.resolution.json'
...
const index = {
  schemaVersion: 1, artifactSetId: dual.prepared.manifest.batchId + '-explicit-informed-six-v1',
...
artifacts.push({path: join(dir, 'resolution-index.informed-six-v1.json'), text: bytes(index)})
artifacts.push({path: join(dir, 'informed-six-materialization.receipt.json'), text: bytes({schemaVersion: 1, authority: 'ai_synthesis', checkedAt, configPath, authoringDigest: sha(fs.readFileSync(authoringPath)), canonicalSnapshotDigest: sha(landscapeBytes), currentAtomicDenominatorSnapshot: denominator, unchangedFullCampaignGoalCount: dual.summary.goalCount, nativeValidatedResolutions: resolutions.map(r=>r.goalId), excluded: spec.excluded, limits: 'Individual native resolution validation; no human approval, no new blind reviews, no claim of all-five-gate completion or final CI. Whole campaigns and non-KEEP findings are preserved.'})})
const { validateLegacyResolutionIndexSnapshot } = await loadModule('app/scripts/reportDeepUnderstandingRollout.ts')
const indexErrors = validateLegacyResolutionIndexSnapshot(index)
if (indexErrors.length) throw Error(indexErrors.join(' | '))
if (checkOnly) {
  for (const artifact of artifacts) {
    const actual = fs.readFileSync(artifact.path, 'utf8')
    if (artifact.path === receiptPath) {
      const oldReceipt = JSON.parse(actual)
      const currentReceipt = JSON.parse(artifact.text)
      for (const key of ['authority', 'configPath', 'authoringDigest', 'unchangedFullCampaignGoalCount', 'nativeValidatedResolutions', 'excluded']) {
        if (JSON.stringify(oldReceipt[key]) !== JSON.stringify(currentReceipt[key])) throw Error('Receipt binding mismatch: ' + key)
      }
      continue
    }
    if (actual !== artifact.text) throw Error('Materialized artifact differs: ' + artifact.path)
  }
  console.log(JSON.stringify({status: 'PASS', nativeValidatedResolutions: exactGoalIds, legacySnapshotIndex: 'PASS', persistedResolutionDigests: 'PASS', historicalCampaignGoalCount: dual.summary.goalCount, humanApproval: false}))
  process.exit(0)
}
