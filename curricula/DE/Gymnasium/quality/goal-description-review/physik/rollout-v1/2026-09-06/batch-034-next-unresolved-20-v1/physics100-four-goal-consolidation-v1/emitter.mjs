...
const affectedIds=[...historicIds,...newIds,ids.amplifier,ids.lifecycle,ids.energy,ids.comparison,ids.risk,ids.evaluation,ids.qvalue,ids.alphaTunnel,ids.nuclearAssessment,ids.switchAssessment,ids.solidParent]
...
function capture(){
 ...
 for(const g of spec.newGoals)add(paths.canonical,['goals',{id:g.id}],g,'New independent assessable competence, no old-ID provenance or copied image.')
 ...
 for(const {file,source,reasons} of groups.values())for(const [k,v] of Object.entries({rationale:reasons.join(' '),reviewedAt:'2026-09-07',reviewer:'codex-physics-b034-four-goal-consolidation-v1'}))add(paths[file],['decisions',{sourceGoalId:source},k],v,'Current individual original-clause adjudication, not a human or coverage approval.')
 ...
 +' for(const review of reviewed){const d=decisions.find(x=>x.sourceGoalId===review.source);if(!d)continue;d.canonicalGoalIds=[...review.canonicalGoalIds];d.rationale=review.rationale;d.reviewedAt="2026-09-07";d.reviewer="codex-physics-b034-four-goal-consolidation-v1";for(let i=mappings.length-1;i>=0;i--)if(mappings[i].legacyGoalId===review.source)mappings.splice(i,1);for(const row of review.mappings)mappings.push({...row});}\n}\n'
 ...
 for(const key of ['heGenerator','byGenerator','hhGenerator','rpGenerator','bwGenerator']){
  const p=paths[key],t=doc(p),line=t.split('\n').find(l=>l.includes('import')&&l.includes('applyPhysicsB040AstroSplitMappings'))
  if(!line)throw Error('Expected existing pure Layer-A source generator import seam '+p)
  span(p,line,line+'\nimport { applyPhysicsB034ConsolidationMappings } from "./physicsB034ConsolidationMappings"','Import only pure Layer-A source-row replay helper.')
  const seam='applyPhysicsB040AstroSplitMappings(decisions, mappings)'
  span(p,seam,seam+'\n  applyPhysicsB034ConsolidationMappings(decisions, mappings)','Apply current source components after older adjudications; no whole generator rewrite.')
 }
 ...
 for(const p of [...viewPaths,atlas]){
  ...
  for(const {n,path} of entries)add(p,[...path,'projectionRole'],'prerequisiteOnly','Retain old ID without projecting the full historical compound into every scope.')
  for(const id of historicIds){
   if(!entries.some(x=>x.n.kind==='goalEntry'&&x.n.goalId===id))add(p,['rootNodes',{kind:'goalEntry',goalId:id}],{kind:'goalEntry',goalId:id,projectionRole:'prerequisiteOnly'},'Direct historical cluster guard prevents inherited target fanout.')
  }
  // New children: even absent targets get explicit prerequisiteOnly so broader roots cannot leak later.
  for(const id of newIds)if(!sourceSupported(scope,id,isAtlas))add(p,['rootNodes',{kind:'goalEntry',goalId:id}],{kind:'goalEntry',goalId:id,projectionRole:'prerequisiteOnly'},'New competence is not a checked target in this state/course/stage; retained solely as prerequisite ID.')
  ...
  const requested=newIds.filter(id=>sourceSupported(scope,id,isAtlas))
  ...
  for(const id of [ids.comparison,ids.energy,ids.risk,ids.evaluation])if(oldRendered.has(id))requested.push(id)
  ...
  const sourceGroups=[
   ...
   {key:'transistor',label:j==='DE-RP'?'Elektronik – gewähltes Wahlthema':j==='DE-BY'?'Ph10.5 – Transistorschalter':'Transistorschaltungen',ids:[ids.switch,ids.flipflop,ids.amplifier]},
   {key:'stars',label:j==='DE-BY'?'Ph13-GA-ASTRO.4 – Gewählter Astrophysikzweig':'Sternentwicklung und Materiekreislauf',ids:[ids.lifecycle,ids.elements,ids.endstage]},
   {key:'nuclear',label:'Kernreaktionen: Grundlagen',ids:[ids.comparison,ids.energy,ids.risk,ids.evaluation]}
  ]
 ...
 const date='2026-09-07',reviewer='codex-physics-b034-four-goal-consolidation-v1'
 ...
 for(const id of affectedIds){
  ...
  const kind=historicIds.includes(id)?'curricularArea':newIds.includes(id)?'curricularAtomic':old?.semanticKind
 ...
 for(const [k,delta] of [['curricularAtomic',2],['curricularArea',4],['total',6]])add(paths.kinds,['counts',k],kinds.counts[k]+delta,'Six genuinely new atoms and four historical atoms retained as subject clusters.')
 const manifest='app/scripts/config/goal-books/de-gym-physics-national-atlas.sources.json'
 add(manifest,['expectedCurricularAtomicGoalCount'],doc(manifest).expectedCurricularAtomicGoalCount+2,'Net two ordinary atoms; no test-count rewrites.')
 const profileImpact=[]
 ...
 return {schemaVersion:1,packageId:spec.packageId,status:'authored_candidate_not_applied',capturedAt:new Date().toISOString(),reviewer:{agent:'physics_b043_blind_b',provider:'unknown',model:'unknown',modelVersion:'unknown'},ids,sourceBindings:spec.originalSources.map(x=>({...x,sha256:sha(readFileSync(resolve(repo,x.path)))})),officialByExperimentSelection:spec.officialByExperimentSelection,operations:ops,positiveUnderstandingEvidence:evidence,sourceProfileConstraints:spec.originalClauseConstraints,viewAudit,profileImpact,cardImpact:{keptOriginRows:affectedCards.length,changes:[],reason:'No kept card has an origin at any converted cluster or refined amplifier. New seven ordinary-atomic decisions individually need no new memory unit.'},notAuthorized:['D/P registration or recognition','Old blind seal changes','Image generation/import','Runtime changes','Old canonical ID-to-child mastery copying'],assessmentLimitations:['Concrete switch assessment maps only to switch; concrete nuclear assessment maps comparison/energy/evaluation only.','Generic Q4 capstone and generic nuclear/solid-state templates retain historical references; generic task wording cannot prove per-child assessment.','No existing concrete assessment located for optical model, flipflop, heavy-element argument or mass-loss forecast. Future assessment authoring remains a separate acceptance task.'],coverageLimitations:['BY reactor schematic/function is not fully represented by the reused reaction comparison/energy/evaluation atoms; partial source coverage is reported, never papered over by a new near-duplicate goal.','No nationwide original-source completeness or full M6 claim from this scoped authoring package.']}
}
let plan
...
for(const id of newIds){const g=byId.get(id);if(g.contains.length||g.extendedData?.splitFromCanonicalGoalId||g.resourceLinks?.length)throw Error('New atom independence/image error')}
...
for(const p of [...viewPaths,atlas]){
 ...
 for(const id of newIds){const wanted=sourceSupported(v.scope??{},id,p===atlas);if(roles.targetGoalIds.has(id)!==wanted||((counts.get(id)??0)!==(wanted?1:0)))throw Error('New target leakage/duplicate/missing '+p+' '+id)}
