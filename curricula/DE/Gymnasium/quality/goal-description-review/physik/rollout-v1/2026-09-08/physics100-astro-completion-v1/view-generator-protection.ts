...
const assert=(v:any,message:string)=>{if(!v)throw Error('B040 view protection: '+message)}
export function preserveB040PlacementsFromCurrent(draft:any,current:any){
 ...
 const collect=(nodes:any[],ancestors:any[]=[])=>nodes.forEach(n=>{
  if(ids.has(n.goalId))placements.push({node:structuredClone(n),ancestors})
  collect(n.children??[],[...ancestors,n])
 })
 collect(current.rootNodes)
 const remove=(nodes:any[]):any[]=>nodes.filter(n=>!ids.has(n.goalId)&&!oldParents.has(n.goalId)).map(n=>{
 ...
 for(const placement of placements){
  ...
  for(const ancestor of placement.ancestors){
   ...
   let found=siblings.find((n:any)=>n.kind==='structure'&&n.id===ancestor.id)
   if(!found){
...
export function preservePhysicsB040ViewPlacements(root:string,path:string,draft:any){
