...
- the upper-secondary NRW lane now also reaches the retained E-phase power/polynomial split through a reviewed bridge on the shared canonical `Funktionen und ihre Darstellung` cluster; with that retained E-parent and the Q-phase exponential parent now both anchored, the current NRW upper-secondary pilot subset is exhausted at explicit parent-cleanup level and further NRW widening should come from another equally explicit imported source corridor
