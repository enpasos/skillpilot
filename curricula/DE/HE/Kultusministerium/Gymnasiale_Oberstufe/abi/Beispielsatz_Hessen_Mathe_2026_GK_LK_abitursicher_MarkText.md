...
> **Leitidee:** Auch kurze Aufgaben (Teil 1) verlangen **eigene Strukturierung**; Aufgaben mit Tool‑Einsatz (Teil 2) verlangen **Setup + Ergebnisdeutung + Plausibilitätscheck**.

---

## Allgemeine Hinweise (für beide Niveaus)

- **Teil 1 (hilfsmittelfrei):** keine WTR/CAS; zugelassen sind die im Abitur üblichen Hilfsmittel (z. B. Wörterbuch Rechtschreibung).  
- **Teil 2 (mit Hilfsmitteln):** WTR/CAS + Formelsammlung.  
- **Dokumentation bei Tool‑Aufgaben:**  
  Bei rechnergestützten Bestimmungen sind **Ansatz/Modellgleichung**, **Eingabe(n)**, **Ausgabe(n)** und **ein kurzer Plausibilitätscheck/Interpretation** anzugeben.  
  Reine „Output‑Zahlen“ ohne Einordnung werden nicht voll gewertet.
...
2. Deuten Sie die **Steigung** dieser Tangente im Kontext „momentane Änderungsrate“ in **einem Satz**. (1 BE)

*(Hinweis: Der Lösungsweg soll selbst strukturiert werden; keine Teilaufgabe „erst ableiten“.)*
...
2. Entscheiden Sie, ob der Punkt $P(2,1,0)$ auf $E$ liegt, und begründen Sie rechnerisch. (3 BE)
...
2. Bestimmen Sie den **Winkel** zwischen den Richtungsvektoren (geben Sie einen exakten Wert an). (2 BE)
...
2. Stellen Sie eine Bestandsfunktion $B(t)$ auf und geben Sie $B(t)$ in möglichst einfacher Form an. (6 BE)  
3. Bestimmen Sie den Zeitpunkt, an dem $r(t)$ maximal ist, und interpretieren Sie das Ergebnis im Kontext. (6 BE)  
4. Bestimmen Sie $\lim_{t\to\infty} r(t)$ und deuten Sie das Ergebnis. (3 BE)  
5. **WTR/CAS:** Bestimmen Sie näherungsweise den Zeitpunkt $t$, zu dem erstmals $B(t)=25$ gilt.  
   Dokumentation: Gleichung/Ansatz + Tool‑Eingabe/Output + kurzer Plausibilitätscheck (z. B. durch Einsetzen/Graph). (7 BE)
...
2. **WTR/CAS:** Bestimmen Sie den Ablehnungsbereich zum Signifikanzniveau $\alpha=5\%$.  
...
4. **Vergleich (inverse Idee):** Unter der Annahme $p=0{,}03$ wird die relative Häufigkeit $\hat p$ näherungsweise normalverteilt modelliert.  
   Vergleichen Sie die (ungefähre) **Breite** eines 95%-Intervalls für $\hat p$ für $n=200$ und $n=800$ (Tool erlaubt).  
   Ein kurzer Rechnungsgang + ein Satz Interpretation. (2 BE)
...
*(Hinweis: Begründen Sie kurz, warum hier „Abstand paralleler Geraden“ sinnvoll ist.)*
...
*(Hinweis: Es ist eine Vorzeichenbetrachtung erforderlich.)*
...
3. Interpretieren Sie den Vergleich von $v_2$ und $v$ in einem Satz (Tendenz der langfristigen Entwicklung). (1 BE)
...
2. Entscheiden Sie **ohne Rechner** plausibel, ob das Ergebnis eher gegen $H_0$ spricht, und begründen Sie kurz (z. B. Vergleich mit Erwartungswert und Streuung). (2 BE)
...
2. Bestimmen Sie $r$ (Tool ist erlaubt, aber dokumentieren). (6 BE)  
3. Zeigen Sie: Die Wachstumsrate $N'(t)$ ist genau dann maximal, wenn $N(t)=\frac{K}{2}$ gilt. (8 BE)  
4. **WTR/CAS:** Bestimmen Sie den Zeitpunkt $t$, zu dem $N(t)=900$ erreicht ist. Dokumentation Pflicht. (6 BE)  
...
3. Treffen Sie die Testentscheidung und formulieren Sie das Ergebnis im Kontext. (4 BE)  
...
5. **Normalverteilung inkl. inverse Fragestellung (WTR/CAS):**  
   ...
   Dokumentation + kurzer Plausibilitätscheck (z. B. Lage von $c$ relativ zu $\mu$). (3 BE)
