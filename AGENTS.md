...
* Goal-visualization QA is tracked per subject under `curricula/DE/Gymnasium/quality/goal-visualization-qa/*.qa.json`. `CQR-303` now represents complete deep curriculum QA, as defined in `docs/concept/curriculum-quality-and-human-trial.md`: M6 plus the central strict D/P/A/M/V intersection for every current `curricularAtomic` goal, successful required validation checks and no relevant blockers. Images are gate V, not a separate highest maturity. Technical `deferred_provider_limitation` is unfinished work and never a completed exception. An explicit `Approved AI` decision remains AI evidence and requires an exact current asset hash; it never becomes human approval. Explicit human error findings prevail. Existing human image approvals, historical artifacts and mandatory human release gates remain intact. M7 grants no human approval or practical human-trial completion and does not weaken core M5 or memory M6 requirements.
