...
* For new Mathematik and Physik visualizations, Nano Banana Pro is the default production provider; its loose, approachable style is intentional. Preserve an existing fachlich correct Nano-Banana-Pro image instead of replacing it merely because a repository-native asset would be easier to control. Apply this rule retroactively to every learner-facing Mathematik or Physik image newly created during an unfinished rollout, including earlier checkpoints of that rollout.
...
* If repeated targeted Nano Banana Pro attempts for one goal remain fachlich wrong, normally remove the active `goal-visualization` link and published asset copies, mark the goal as `deferred_provider_limitation` in the review ledger, and revisit when the provider improves. A precise repository-native fallback is permitted only as a narrow documented exception when the required correctness or legibility cannot be achieved with Nano Banana Pro. Its review record must bind the attempted prompts or reference strategy, the visible defects of the rejected candidates, and the reason the fallback is necessary; an easier implementation alone is not sufficient.
* When an atomic goal is split, retain a good existing aggregate Nano-Banana-Pro image on the surviving parent cluster whenever it still correctly summarizes the combined child content. Add more specific child images separately instead of erasing the useful overview.
