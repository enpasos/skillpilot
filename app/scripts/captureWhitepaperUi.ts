...
try {
  for (const language of ['de', 'en'] as const) {
    for (const view of ['plan', 'materials'] as const) {
      ...
      if (view === 'plan') {
        const heading = capture.getByRole('heading', { name: language === 'de' ? 'Lernplan' : 'Learning plan', exact: true })
        await heading.waitFor()
        assert.match(await heading.getAttribute('class') ?? '', /\bsr-only\b/u,
          'the overview heading labels the region without repeating the dial period visibly')
        ...
        await math.getByTestId('learner-plan-period-gauge').getByText(language === 'de' ? 'Heute' : 'Today', { exact: true }).waitFor()
        await math.getByTestId('learner-plan-period-gauge').getByText(language === 'de' ? '1 von 3' : '1 of 3', { exact: true }).waitFor()
        ...
        await physics.getByTestId('learner-plan-period-gauge').getByText(language === 'de' ? 'Heute' : 'Today', { exact: true }).waitFor()
        await physics.getByTestId('learner-plan-period-gauge').getByText(language === 'de' ? '2 von 2' : '2 of 2', { exact: true }).waitFor()
        ...
        assert.equal(await capture.getByRole('button', { name: language === 'de' ? 'Weiterlernen' : 'Continue learning' }).count(), 0)
        assert.equal(await capture.getByRole('button', { name: language === 'de' ? 'Einstellungen öffnen' : 'Open settings' }).count(), 0)
        const switchBadge = physics.getByTestId('learner-plan-switch')
        await switchBadge.getByText(language === 'de' ? 'Wechseln' : 'Switch', { exact: true }).waitFor()
        assert.equal(await switchBadge.getAttribute('aria-label'), language === 'de' ? 'Zu Physik wechseln' : 'Switch to Physics')
        assert.equal(await switchBadge.isEnabled(), true)
