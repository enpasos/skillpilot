  ...
  for (const language of ['de', 'en'] as const) {
    for (const view of ['plan', 'materials'] as const) {
      ...
      if (view === 'plan') {
        ...
        await math.getByText(language === 'de' ? '1 von 3 Zielen' : '1 of 3 goals', { exact: true }).waitFor()
        ...
        await physics.getByText(language === 'de' ? '2 von 2 Zielen' : '2 of 2 goals', { exact: true }).waitFor()
