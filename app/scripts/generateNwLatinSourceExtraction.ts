...
const specs: ExtractionSpec[] = [
  {
    extractionId: 'DE_NW_LATEIN_SEKI_KLP2019',
    title: 'Latein Sekundarstufe I (Nordrhein-Westfalen, KLP 2019 Source-Extraction)',
    sourceLandscapeId: uuidFromString('DE-NW-LATEIN-SEKI-KLP2019-SOURCE-EXTRACTION'),
    sourceDocumentKey: 'NW-KLP-LATEIN-SEKI-2019',
    sourceDocumentTitle: 'Nordrhein-Westfalen Kernlehrplan Latein Sekundarstufe I Gymnasium 2019',
    sourcePdfPath: 'curricula/DE/Gymnasium/input/NW/latein/g9_l_klp_3402_2019_06_23_0.pdf',
    sourceUrl: 'https://lehrplannavigator.nrw.de/system/files/media/document/file/g9_l_klp_3402_2019_06_23_0.pdf',
    stage: 'SekI',
    extractionPath:
      'curricula/DE/Gymnasium/input/NW/latein/source-extraction/DE_NW_LATEIN_SEKI_KLP2019.source-extraction.json',
    reviewPath:
      'curricula/DE/Gymnasium/mapping/DE-NW/lower-secondary/nw_latin_lower_secondary_source_extraction_to_canonical_latin.review.json',
    firstPage: 16,
    lastPage: 31,
    expectedPassages: 18,
    expectedMinimumGoals: 80,
    sourceGoalCountReview:
      'NRW Sek I konsolidiert Latein als zweite Fremdsprache, Latein ab Jahrgangsstufe 5 und Latein als dritte Fremdsprache in einem KLP. Die geringere Zielzahl gegenueber BW ist plausibel, weil BW vier getrennte Bildungsgang-PDFs mit wiederholten Kompetenzlisten fuehrt.',
  },
  {
    extractionId: 'DE_NW_LATEIN_SEKII_KLP2013',
    title: 'Lateinisch Oberstufe (Nordrhein-Westfalen, KLP 2013 Source-Extraction)',
    sourceLandscapeId: uuidFromString('DE-NW-LATEIN-SEKII-KLP2013-SOURCE-EXTRACTION'),
    sourceDocumentKey: 'NW-KLP-LATEIN-SEKII-2013',
    sourceDocumentTitle: 'Nordrhein-Westfalen Kernlehrplan Lateinisch Gymnasiale Oberstufe 2013',
    sourcePdfPath: 'curricula/DE/Gymnasium/input/NW/latein/klp_gost_lateinisch.pdf',
    sourceUrl: 'https://lehrplannavigator.nrw.de/system/files/media/document/file/klp_gost_lateinisch.pdf',
    stage: 'SekII',
    extractionPath:
      'curricula/DE/Gymnasium/input/NW/latein/source-extraction/DE_NW_LATEIN_SEKII_KLP2013.source-extraction.json',
    reviewPath:
      'curricula/DE/Gymnasium/mapping/DE-NW/upper-secondary/nw_latin_upper_secondary_source_extraction_to_canonical_latin.review.json',
    firstPage: 20,
    lastPage: 45,
    expectedPassages: 36,
    expectedMinimumGoals: 150,
    sourceGoalCountReview:
      'NRW Sek II enthaelt fortgefuehrtes und neu einsetzendes Latein, jeweils mit EF/Q-Progression und inhaltsfeldbezogenen Kompetenzlisten. Die Zielzahl liegt deutlich ueber HE Sek II, weil NRW mehr Kursvarianten explizit ausweist.',
  },
]
...
function parseBullets(spec: ExtractionSpec): ParsedBullet[] {
  ...
  let phase = spec.stage === 'SekI' ? 'Latein als zweite Fremdsprache - erste Stufe' : 'Fortgefuehrte Fremdsprache - Einfuehrungsphase'
  let field = 'Uebergreifende Kompetenzerwartungen'
  let collecting = false
  let current: { text: string, bulletSymbol: string } | null = null
  ...
  const flush = () => {
    ...
    if (normalized.length > 0) {
      bullets.push({
        ...
        bulletSymbol: current.bulletSymbol,
      })
    }
    current = null
  }
  ...
  for (const rawLine of text.split(/\r?\n/u)) {
    ...
    if (/^Die Schülerinnen und Schüler können/u.test(line)) {
      flush()
      collecting = true
      continue
    }
    ...
    const bulletMatch = line.match(/^\s*([à■□•])\s+(.+)$/u)
    if (bulletMatch && collecting) {
      flush()
      current = {
        bulletSymbol: bulletMatch[1],
        text: bulletMatch[2] ?? '',
      }
      continue
    }

    if (current && collecting && !looksStructural(line)) {
...
function buildDocuments(spec: ExtractionSpec, bullets: ParsedBullet[]) {
  ...
  const decisions = bullets.map((bullet) => {
    ...
    return {
      ...
      rationale: canonicalGoalIdsForSource.length > 0
        ? `Das NRW-Latein-Source-Ziel "${sourceGoal.title}" ist inhaltlich durch kanonische Latein-Ziele abgedeckt. 1:1 und 1:n sind gleichwertige Abdeckungsformen; partial bedeutet keine offene Luecke.`
        : `Das NRW-Latein-Source-Ziel "${sourceGoal.title}" ist noch nicht belastbar durch kanonische Latein-Ziele abgedeckt.`,
      reviewedAt,
      reviewer: 'Codex',
    }
  })
  ...
  const extraction = {
    ...
    method: {
      ...
      mappingStatus: m3Complete
        ? 'MAPPING-3 complete after fachliche review: all official NRW Latin source goals are covered by canonical Latin goals.'
    ...
    qualityReview: {
      sourceGoalCountPeerBaseline: {
        ...
        rationale:
          'Die Source-Ziel-Anzahl wurde kritisch gegen die Struktur der amtlichen NRW-KLP-PDFs geprueft; gezaehlt werden nur Kompetenzbullets, keine Inhaltslisten.',
      },
    },
  ...
  const review = {
    ...
    summary: {
      ...
      inheritedMappings: 0,
      note: m3Complete
        ? 'NRW Latein ist fachlich erstgeprueft: alle amtlichen Source-Ziele sind durch kanonische Latein-Ziele abgedeckt. 1:1 und 1:n sind gleichwertig; partial beschreibt die Zuordnungsform, nicht eine fachliche Luecke.'
        : 'NRW Latein ist fachlich erstgeprueft; explizite Canonical-Gaps bleiben offen.',
    },
    mappings,
    decisions,
  }
...
function canonicalTargetsForSourceGoal(sourceGoal: SourceGoal): string[] {
  ...
  if (/sprachkompetenz|sprachsystem|sprache|grammatik|morpholog|syntax|satz|wort|fachterminologie|fachsprach|fremdwort|lehnwort|wortschatz|wortbildung|systemgrammatik/u.test(`${field} ${text}`)) {
...
function addLowerSecondaryTargets(text: string, targets: Set<string>): void {
  if (/aussprache|basiswortschatz|grundwortschatz|wortschatz|vokabel|wortbildung|wortart|wortfamilie|wortfeld|lehn|fremdwort|fremdsprache/u.test(text)) {
    ...
    if (/wortart|flexionsklasse|strukturieren|basiswortschatz|grundwortschatz/u.test(text)) {
      targets.add(canonicalGoalIds.vocabularyOrderLexicalData)
    }
    if (/deutsch|fremdwort|lehnwort|fremdsprachen|sprachverwandt|lateinische ausgangsform/u.test(text)) {
      targets.add(canonicalGoalIds.vocabularyLanguageConnections)
    }
  }

  if (/morpholog|formenaufbau|flexionsklasse|konjugation|deklination|kasus|tempus|modus|personalendung|verbform|form und funktion/u.test(text)) {
...
function phaseForLine(spec: ExtractionSpec, line: string): string | null {
  if (spec.stage === 'SekI') {
    if (/^2\.2\.1\s+Erste Stufe/u.test(line)) return 'Latein als zweite Fremdsprache - erste Stufe'
    if (/^2\.2\.2\s+Zweite Stufe/u.test(line)) return 'Latein als zweite Fremdsprache - zweite Stufe'
    if (/^2\.3\s+Latein ab Jahrgangsstufe 5/u.test(line)) return 'Latein ab Jahrgangsstufe 5'
    if (/^2\.4\s+Latein als dritte Fremdsprache/u.test(line)) return 'Latein als dritte Fremdsprache'
    return null
  }
  if (/^2\.2\.1\s+Kompetenzerwartungen bis zum Ende der Einf/u.test(line)) {
    return 'Fortgefuehrte Fremdsprache - Einfuehrungsphase'
  }
  if (/^2\.2\.2\s+Kompetenzerwartungen bis zum Ende der Qualifikationsphase im/u.test(line)) {
    return 'Fortgefuehrte Fremdsprache - Grundkurs'
  }
  if (/^2\.2\.3\s+Kompetenzerwartungen bis zum Ende der Qualifikationsphase im/u.test(line)) {
    return 'Fortgefuehrte Fremdsprache - Leistungskurs'
  }
  if (/^2\.3\.1\s+Kompetenzerwartungen bis zum Ende der Einf/u.test(line)) {
    return 'Neu einsetzende Fremdsprache - Einfuehrungsphase'
  }
  if (/^2\.3\.2\s+Kompetenzerwartungen bis zum Ende der Qualifikationsphase/u.test(line)) {
    return 'Neu einsetzende Fremdsprache - Qualifikationsphase'
  }
  return null
}

function fieldForLine(spec: ExtractionSpec, line: string): string | null {
  const sekiCompetencies = ['Textkompetenz', 'Sprachkompetenz', 'Kulturkompetenz']
  if (spec.stage === 'SekI' && sekiCompetencies.includes(line)) return line
  const sekiFieldMatch = line.match(/^Inhaltsfeld\s+\d+:\s+(.+)$/u)
  if (spec.stage === 'SekI' && sekiFieldMatch) return `Inhaltsfeld: ${sekiFieldMatch[1]}`

  const upperLine = line.toUpperCase()
  if (upperLine === 'TEXTKOMPETENZ') return 'Textkompetenz'
  if (upperLine === 'SPRACHKOMPETENZ') return 'Sprachkompetenz'
  if (upperLine === 'KULTURKOMPETENZ') return 'Kulturkompetenz'
  const sekIiFields = [
  ...
  if (spec.stage === 'SekII' && sekIiFields.includes(line)) return `Inhaltsfeld: ${line}`
  return null
}
...
function isStopLine(line: string): boolean {
  return /^(3\s+Lernerfolgs|Lernerfolgsüberprüfung|Beurteilungsbereich|Inhaltliche Schwerpunkte|In den nachfolgenden|▷◁)/u.test(line)
}

function looksStructural(line: string): boolean {
  return isStopLine(line)
    || /^(Kompetenzbereiche, Inhaltsfelder|Lateinisch als|Latein als|Aufgaben und Ziele des Faches|Anhang|Tabelle im Anhang)/u.test(line)
    || /^[–-]\s/u.test(line)
}

function isPageNoise(line: string): boolean {
  return /^\d+$/u.test(line)
    || /^(Kompetenzbereiche, Inhaltsfelder und Kompetenzerwartungen|Lateinisch als fortgeführte Fremdsprache|Lateinisch als neu einsetzende Fremdsprache)$/u.test(line)
}
...
function updateRegistry(): void {
  ...
  const entries = specs.map((spec) => ({
    ...
    archivePath: 'curricula/DE/Gymnasium/input/NW/latein/',
    sourceDocumentKey: spec.sourceDocumentKey,
