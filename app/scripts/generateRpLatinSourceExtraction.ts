...
const specs: Spec[] = [
  {
    stage: 'SekI',
    sourceLandscapeId: uuidFromString('DE-RP-LATEIN-SEKI-GYM-2009-SOURCE-EXTRACTION'),
    extractionId: 'DE_RP_LATEIN_SEKI_GYM_2009',
    title: 'Latein Sekundarstufe I (Rheinland-Pfalz, Lehrplan 2009 Source-Extraction)',
    sourceDocumentKey: 'RP-LATEIN-SEKI-GYM-2009',
    sourceDocumentTitle: 'Rheinland-Pfalz Lehrplan Latein Sekundarstufe I, 2009',
    sourcePdfPath: 'curricula/DE/Gymnasium/input/RP/latein/Latein_Sekundarstufe_I.pdf',
    sourceUrl:
      'https://bildung.rlp.de/lehrplaene/?tx_rlpbase_download%5Baction%5D=download&tx_rlpbase_download%5Bcontroller%5D=Download&tx_rlpbase_download%5Bitem%5D=56497&type=432522',
    extractionPath:
      'curricula/DE/Gymnasium/input/RP/latein/source-extraction/DE_RP_LATEIN_SEKI_GYM_2009.source-extraction.json',
    reviewPath:
      'curricula/DE/Gymnasium/mapping/DE-RP/lower-secondary/rp_latin_lower_secondary_source_extraction_to_canonical_latin.review.json',
    blocks: [
      { from: 12, to: 26, phase: 'Latein I (erste Fremdsprache)', area: 'Sek-I-Standards' },
      { from: 29, to: 42, phase: 'Latein II (zweite Fremdsprache)', area: 'Sek-I-Standards' },
      { from: 46, to: 54, phase: 'Latein III (dritte Fremdsprache)', area: 'Sek-I-Standards' },
    ],
    expectedMinimumGoals: 200,
    expectedPassages: 10,
    methodDescription:
      'Extrahiert die nummerierten Kompetenzstandards und expliziten Tabellen-/Listeninhalte aus den amtlichen RLP-Bildungsgaengen Latein I, II und III.',
    completenessNote:
      'Der RLP-Sek-I-Lehrplan 2009 ist nach Latein I, Latein II und Latein III getrennt. Die Extraktion erfasst die verbindlichen Standards je Bildungsgang und zaehlt reine Textvorschlaege nicht als Source-Ziele.',
  },
  {
    stage: 'SekII',
    sourceLandscapeId: uuidFromString('DE-RP-LATEIN-SEKII-GYO-1998-SOURCE-EXTRACTION'),
    extractionId: 'DE_RP_LATEIN_SEKII_GYO_1998',
    title: 'Latein Oberstufe (Rheinland-Pfalz, Lehrplan 1998 Source-Extraction)',
    sourceDocumentKey: 'RP-LATEIN-SEKII-GYO-1998',
    sourceDocumentTitle: 'Rheinland-Pfalz Lehrplan Latein Gymnasiale Oberstufe, 1998',
    sourcePdfPath: 'curricula/DE/Gymnasium/input/RP/latein/Latein_Sekundarstufe_II.pdf',
    sourceUrl:
      'https://bildung.rlp.de/lehrplaene/?tx_rlpbase_download%5Baction%5D=download&tx_rlpbase_download%5Bcontroller%5D=Download&tx_rlpbase_download%5Bitem%5D=56500&type=432522',
    extractionPath:
      'curricula/DE/Gymnasium/input/RP/latein/source-extraction/DE_RP_LATEIN_SEKII_GYO_1998.source-extraction.json',
    reviewPath:
      'curricula/DE/Gymnasium/mapping/DE-RP/upper-secondary/rp_latin_upper_secondary_source_extraction_to_canonical_latin.review.json',
    blocks: [
      { from: 7, to: 53, phase: 'Gymnasiale Oberstufe', area: 'Allgemeine Ziele und Themenziele' },
    ],
    expectedMinimumGoals: 130,
    expectedPassages: 16,
    methodDescription:
      'Extrahiert die allgemeinen Lernbereiche und die expliziten Ziel-Listen der RLP-Oberstufenthemen; Textvorschlaege bleiben Quellenkontext und werden nicht als Lernziele gezaehlt.',
    completenessNote:
      'Der RLP-Sek-II-Lehrplan ist literatur- und themenorientiert. Die Extraktion wertet alle mit "Ziele" markierten Themenpassagen sowie die fachspezifisch konkreten Lernbereiche aus.',
  },
];
...
function parseSpec(spec: Spec): ParsedGoal[] {
  if (spec.stage === 'SekI') {
    return spec.blocks.flatMap((block) => parseSekIBlock(spec, block));
  }
  return spec.blocks.flatMap((block) => parseSekIIBlock(spec, block));
}

function parseSekIBlock(spec: Spec, block: Block): ParsedGoal[] {
  const text = pdftotext(spec.sourcePdfPath, block.from, block.to);
  const goals: ParsedGoal[] = [];
  const currentPhase = block.phase;
  ...
  const flush = () => {
    ...
    goals.push({
      ...
      sourceText,
      sourceLocator: `S. ${currentPage}, ${currentPhase}, ${currentArea}${currentCode ? `, ${currentCode}` : ''}`,
    });
  };

  for (const rawLine of text.split(/\r?\n/u)) {
    const line = normalizeWhitespace(rawLine.replace(/[]/gu, '•').replace(/^[-–]\s+/u, '• '));
    ...
    if (/^(Progressionspfeil|Standard nach|Die Schülerinnen und Schüler:|Lektüre-Verbindlichkeiten)/u.test(line)) {
      flush();
      continue;
    }

    const numbered = line.match(/^(\d\.\d(?:\.\d+)?)\s+(Die\s+Schülerinnen\s+und\s+Schüler\s+.+)$/u);
    if (numbered) {
      flush();
      currentCode = numbered[1];
      current = [numbered[2]];
      continue;
    }

    const area = detectArea(line, block.area, currentArea);
    if ((isHeadingLine(line) || area !== currentArea) && !hasBullet(line)) {
      flush();
      currentArea = area;
      continue;
    }
    ...
    if (shouldPromoteLine(spec, line)) {
      flush();
      currentCode = '';
      current = [line];
      flush();
...
function parseSekIIBlock(spec: Spec, block: Block): ParsedGoal[] {
  const text = pdftotext(spec.sourcePdfPath, block.from, block.to);
  const goals: ParsedGoal[] = [];
  let currentPhase = block.phase;
  let currentArea = block.area;
  let currentPage = `${block.from}-${block.to}`;
  let currentTopic = 'Allgemeine Ziele';
  let current: string[] | null = null;
  let inGeneralGoals = false;
  let inThemeGoals = false;
  ...
  const flush = () => {
    ...
    if (!isUsableGoalText(sourceText)) return;
    goals.push({
      passageId: slug(`${spec.extractionId}-${currentTopic}-${currentArea}`),
      phase: currentPhase,
      area: currentArea,
      title: shortTitle(sourceText),
      description: asCanStatement(sourceText),
      sourceText,
      sourceLocator: `S. ${currentPage}, ${currentTopic}, ${currentArea}`,
    });
  };
  ...
  for (const rawLine of text.split(/\r?\n/u)) {
    ...
    if (/^1\.5\s+Methodische Anmerkungen/u.test(line)) {
      flush();
      inGeneralGoals = false;
      continue;
    }
    if (/^6\.\s+Fachübergreifendes/u.test(line)) {
      flush();
      break;
    }

    const learningArea = line.match(/^Lernbereich\s+(.+)$/u);
    if (learningArea) {
      flush();
      currentTopic = 'Fachspezifisch konkrete Ziele';
      currentArea = `Lernbereich ${learningArea[1]}`;
    ...
    const topic = line.match(/^(Thema\s+(?:[IVX]+\s*)?(?:\d+|[A-Z])(?:\s*\([^)]+\))?:\s+.+)$/u);
    if (topic) {
      flush();
      currentTopic = cleanSourceText(topic[1]);
      currentArea = detectArea(currentTopic, block.area, currentArea);
      currentPhase = block.phase;
      inGeneralGoals = false;
      inThemeGoals = false;
      continue;
    ...
    if (/^(Ziele|Ziele für das Grundfach|[Zz]usätzliche?s? Ziele? für das Leistungsfach)$/u.test(line)) {
      flush();
      inThemeGoals = true;
      inGeneralGoals = false;
      if (/Leistungsfach/u.test(line)) currentPhase = 'Gymnasiale Oberstufe · Leistungsfach';
      continue;
    }

    if (/^(Textvorschläge|Textvorschlag|Auswahlvorschläge|A:|B:|C:|Addita?:|Alle Vorschläge|Eine Kombination|Exemplarisch|Die Reihenfolge)/u.test(line)) {
      flush();
      inThemeGoals = false;
      continue;
    }

    if (/^Ziel der Lektüre /u.test(line)) {
      flush();
      current = [line];
      flush();
      continue;
    ...
    const bulletParts = extractBulletParts(line);
    if (bulletParts.length > 0 && (inGeneralGoals || inThemeGoals)) {
      for (const bullet of bulletParts) {
        if (isTextRecommendation(bullet)) continue;
        flush();
        current = [bullet];
      }
      continue;
    ...
    if (current && (inGeneralGoals || inThemeGoals) && !isRightColumnHint(line)) {
      current.push(line);
      continue;
    }
  }
...
function detectArea(line: string, blockArea: string, fallback: string): string {
  if (/^\d\s+KOMPETENZBEREICH SPRACHE/u.test(line)) return 'Kompetenzbereich Sprache';
  if (/^1\.1\s+WORTSCHATZ/u.test(line)) return 'Kompetenzbereich Sprache · Wortschatz';
  if (/^1\.2\s+GRAMMATIK/u.test(line)) return 'Kompetenzbereich Sprache · Grammatik Formenlehre';
  if (/^1\.3\s+GRAMMATIK/u.test(line)) return 'Kompetenzbereich Sprache · Grammatik Syntax';
  if (/^\d\s+KOMPETENZBEREICH TEXT/u.test(line)) return 'Kompetenzbereich Text';
  if (/^2\.1\s+ERSCHLIE/u.test(line)) return 'Kompetenzbereich Text · Erschließen';
  if (/^2\.2\s+ÜBERSETZEN/u.test(line)) return 'Kompetenzbereich Text · Übersetzen';
  if (/^2\.3\s+INTERPRETIEREN/u.test(line)) return 'Kompetenzbereich Text · Interpretieren';
  if (/^\d\s+KOMPETENZBEREICH KULTUR/u.test(line)) return 'Kompetenzbereich Kultur';
  if (/^Lektüre$/u.test(line)) return 'Lektüre';
  if (/^Thema\s+\d+:\s+Römische Rhetorik/u.test(line)) return 'Jahrgangsstufe 11 · Rhetorik';
  if (/^Thema\s+\d+:\s+(Sallust|Tacitus|Plinius)/u.test(line)) return 'Jahrgangsstufe 11 · Prosa';
  if (/^Thema\s+\d+:\s+(Catull|Ovid|Die römische Komödie|Satirische Texte)/u.test(line)) {
    return 'Jahrgangsstufe 11 · Dichtung und Literaturformen';
  }
  if (/^Thema\s+III/u.test(line)) return 'Jahrgangsstufen 12/13 · Römisches Philosophieren';
  if (/^Thema\s+II/u.test(line)) return 'Jahrgangsstufen 12/13 · Augusteische Dichtung';
  if (/^Thema\s+I(?:\s|$)/u.test(line)) return 'Jahrgangsstufen 12/13 · Historisch-politische Prosa';
  ...
  if (/^(Römische Dichtung|Motive und ihre Hintergründe|Grundlagen römischer Metrik)$/u.test(line)) {
...
function isNoiseLine(line: string): boolean {
  return (
    ...
    /^Rheinland-Pfalz$/u.test(line) ||
    /^Lehrplan\s+Latein/u.test(line) ||
...
function toSourceGoal(spec: Spec, goal: ParsedGoal, index: number): SourceGoal {
  ...
  return {
    ...
    tags: ['jurisdiction:DE-RP', 'subject:Latein', `stage:${spec.stage}`],
    metadata: {
      phase: goal.phase,
      field: goal.area,
...
function toDecision(spec: Spec, goal: SourceGoal): Decision {
  ...
  return {
    ...
    matchType,
    rationale:
      'Das RLP-Latein-Source-Ziel ist inhaltlich durch kanonische Latein-Ziele abgedeckt. 1:1 und 1:n sind gleichwertige Abdeckungsformen; partial bedeutet keine offene Luecke.',
    reviewedAt,
    reviewer,
  };
}
...
function buildExtraction(spec: Spec, passages: Passage[], sourceGoals: SourceGoal[]) {
  ...
  return {
    ...
    method: {
      extractor: 'generateRpLatinSourceExtraction.ts',
...
function buildReview(spec: Spec, sourceGoals: SourceGoal[], decisions: Decision[]) {
  ...
  return {
    ...
    summary: {
      ...
      inheritedMappings: 0,
      note:
        'RLP Latein ist fachlich erstgeprueft: alle amtlichen Source-Ziele sind durch kanonische Latein-Ziele abgedeckt. 1:1 und 1:n sind gleichwertig; partial beschreibt die Zuordnungsform, nicht eine fachliche Luecke.',
    },
    mappings,
    decisions,
  };
...
function upsertRegistry(spec: Spec, sourceGoals: number, passages: number) {
  ...
  const entry = {
    ...
    archivePath: 'curricula/DE/Gymnasium/input/RP/latein/',
    sourceDocumentKey: spec.sourceDocumentKey,
...
function cleanSourceText(text: string): string {
  return normalizeWhitespace(
    text
      ...
      .replace(/\s*→\s*/gu, ' ')
      .replace(/\s*\[[^\]]+\]\s*/gu, ' ')
      .replace(/\s+[Zz]usätzliche?s? Ziele? für das Leistungsfach\s*$/u, '')
      .replace(/\s+\d{1,2}$/u, '')
      .replace(/^\+\s*/u, '')
      .replace(/^[,;]\s*/u, ''),
  ).replace(/[;,]\s*$/u, '');
...
function isUsableGoalText(text: string): boolean {
  ...
  if (/^(Der Inhalt der vorangehenden Spalte|Diese Weiterführung|Ist in der vorangehenden Spalte)/u.test(text)) return false;
  if (/^(INTERPRETIEREN|ÜBERSETZEN|ERSCHLIEßEN|WORTSCHATZ|GRAMMATIK)/u.test(text)) return false;
