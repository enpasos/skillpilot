...
const outputStem = 'stable-current-carryover-5-v1'
...
const sourceHashes = {
  ...
  canonical: '5269c8634d0e839db077fe64e8946ff8934909372921e9844d8e1f5b6826628d',
  semanticKindLedger: '4a2a63ce2352cb1153e2cb3ded462e5c7f76939b8176a28270a69c81c0d77ae5',
} as const
...
const evidenceRoundByGoal = new Map<string, ReviewRound>([
  ...
  [stableGoalIds[3], 'first'],
  [stableGoalIds[4], 'second'],
])
const rationaleByGoal = new Map<string, { de: string; en: string }>([
  ...
  [stableGoalIds[3], {
    de: 'Beide unabhängigen Blindprüfungen bestätigen die inverse Rekonstruktionskompetenz. Runde A wird ausgewählt, weil sie die Formel a = (y - e)(x - d), die vollständige Parametergewinnung und die unabhängige Graphenprüfung am klarsten verbindet; Runde B bleibt vollständig gebunden.',
    en: 'Both independent blind reviews confirm the inverse reconstruction competency. Round A is selected because it most clearly connects the formula a = (y - e)(x - d), complete parameter recovery, and an independent check against the graph; Round B remains fully bound.',
  }],
  [stableGoalIds[4], {
...
const main = async (): Promise<void> => {
  ...
  if (stableGoalIds.some((goalId) => !campaignIds.includes(goalId)) || !campaignIds.includes(revisedGoalId)) {
    throw new Error('Stable or revised scope escaped B025a')
  }
  ...
  const manifestId = 'mathematik-b025a-stable5-synthesis-openai-codex-20260902'
  const manifestPayload: Omit<GoalDescriptionRolloutSynthesisDecisionManifest, 'manifestFingerprint'> = {
    ...
    synthesizedBy: 'OpenAI Codex B025a stable-five adjudicated carryover synthesis candidate',
    synthesizedAt,
  ...
  for (const goal of expectedGoals) {
    ...
    const resolution = buildGoalDescriptionDualRoundResolution({
      resolutionId: `mathematik-b025a-stable5-current-carryover-v1-resolution-${goal.goalId}`,
      goalId: goal.goalId,
      effectiveSemanticKind: 'curricularAtomic',
  ...
  const receiptBody = {
    ...
    purpose: 'Hash-bound carryover of exactly five adjudicated current B025a goals while be18cef8 requires the adopted local wording revision and fresh B025b blind review.',
    source: {
    ...
    explicitlyExcludedFreshGoalIds: [revisedGoalId],
    ...
    safeguards: {
      ...
      stableFiveAndFreshOneScopesDisjointAndComplete: true,
  ...
  console.log(
    `Mathematics B025a stable-five carryover ${writeMode ? 'materialized' : 'valid'}: `
    + `goals=5; synthesis=${digest(synthesisBytes)}; index=${digest(indexBytes)}; receipt=${digest(receiptBytes)}`,
  )
}
