...
const batchDirectory = join(
  repositoryRoot,
  'curricula/DE/Gymnasium/quality/goal-description-review/physik/rollout-v1/2026-09-05/batch-033v-adjudicated-revisions-context-recheck-12-v1',
)
const resultStem = 'physik-rollout-v1-batch-033v-adjudicated-revisions-context-recheck-12-v1-20260905-first-pass'
...
const synthesisPath = join(batchDirectory, 'synthesis-decisions.stable-current-carryover-4-v1.json')
const resolutionIndexPath = join(batchDirectory, 'resolution-index.stable-current-carryover-4-v1.json')
const outputPath = join(
  repositoryRoot,
  'curricula/DE/Gymnasium/quality/goal-evidence/canonical-physics-positive-understanding-evidence-rollout-v1-batch-033v-stable-current-4-v1.candidates.json',
)
const reviewId = 'canonical-physics-positive-evidence-v1-b033v-stable-current-4-v1'
const goalIds = [
...
const archetypes = new Map<string, PositiveGoalEvidenceProfile['archetype']>([
  ['0b4f2020-8486-5372-9cb9-6e59f698ac2d', 'modeling'],
  ['fd9fd8ad-c4a1-5552-9ea0-1878e0636f20', 'modeling'],
  ['741774ef-15fc-4bcf-a370-e2c5cf4257d0', 'modeling'],
  ['3f17d0d2-562d-4c1c-9ebc-1a1a43f28f9c', 'experiment'],
])
...
const main = async (): Promise<void> => {
  ...
  if (
    ...
    || index.groups[0]?.campaignGoalCount !== 12
    || index.groups[0]?.resolvedGoalCount !== 4
  ...
  ) throw new Error('Physics B033v stable4 synthesis or resolution scope is invalid')
  ...
  const output = {
    ...
    reviewer: 'codex-physics-rollout-v1-batch-033v-stable-current-4-positive-understanding-candidate-author-2026-09-05',
    goals: candidates,
  }
  const bytes = jsonBytes(output)
  const current = await readOptional(outputPath)
  if (current && !current.equals(bytes)) throw new Error(`Existing Physics B033v stable4 evidence candidates are stale: ${outputPath}`)
  if (!current && !write) throw new Error(`Missing Physics B033v stable4 evidence candidates: ${outputPath}`)
  if (!current && write) {
    await mkdir(dirname(outputPath), { recursive: true })
    await writeFile(outputPath, bytes, { flag: 'wx' })
  }
  console.log(`${write ? 'Materialized' : 'Verified'} Physics B033v stable4 evidence candidates: ${candidates.length}/4`)
}
