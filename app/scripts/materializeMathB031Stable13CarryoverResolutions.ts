...
const batchName = 'batch-031-atlas-pages-120-146-20-v1'
...
const authoringPath = join(sourceDirectory, 'stable-current-carryover-13-v1.authoring.json')
...
const outputStem = 'stable-current-carryover-13-v1'
...
const stableGoalIds = [
  'e02c58a2-2a32-49bd-b110-7b307e0317fb',
  ...
  '9278bc5e-a77f-5f72-8636-0d0d3e3d32ae',
  'fc047e6e-5d6d-460f-99fc-ade3a23b9a8e',
  '415bd48b-8a76-4d4f-bfdd-d085573e7ac3',
  '21342eb9-9cfd-4fbf-b28e-60aa2b48f702',
  '36728db8-da44-4add-97b8-0fdd7cfd9c41',
  '759485a9-51c0-4261-af7d-caa3c0e5d68b',
  '4ac925cf-3862-4810-be2a-d92efff7d735',
  'ffd1ae26-c461-4439-9b18-d835c8f38e1a',
] as const
...
const writeAllOrRequireExact = async (
  ...
  artifacts.forEach(({ path, bytes }, index) => {
    if (current[index] && !current[index]?.equals(bytes)) {
      throw new Error(`Existing Mathematics B031 stable carryover artifact is stale: ${path}`)
    }
    if (!current[index] && !write) {
      throw new Error(`Missing Mathematics B031 stable carryover artifact: ${path}`)
    }
  })
  if (!write) return
...
const main = async (): Promise<void> => {
  ...
  if (
    ...
    || authoring.carryoverId !== 'mathematik-b031-stable-current-carryover-13-v1-20260905'
  ) {
    throw new Error('Mathematics B031 stable carryover authoring identity is invalid')
  }
  ...
  if (
    ...
    throw new Error('Mathematics B031 carryover authoring scope or order is invalid')
  }
  const campaignGoalIds = dual.prepared.manifest.goalIds
  if (
    campaignGoalIds.length !== 20
    ...
    throw new Error('Mathematics B031 campaign no longer partitions into exact stable and excluded scopes')
  }
  ...
  if (completedAtValues.length === 0 || completedAtValues.some((value) => !Number.isFinite(value))) {
    throw new Error('B031 source runs must have valid completion timestamps')
  }
  ...
  for (const [index, goalId] of stableGoalIds.entries()) {
    ...
    const resolution = buildGoalDescriptionDualRoundResolution({
      resolutionId: `mathematik-b031-stable13-current-carryover-v1-resolution-${goalId}`,
      goalId,
      effectiveSemanticKind: 'curricularAtomic',
      decision: 'keep_current',
      synthesis: {
        synthesisId: `mathematik-b031-stable13-current-carryover-v1-synthesis-${goalId}`,
  ...
  const index = {
    schemaVersion: 1,
    artifactSetId: `${dual.prepared.manifest.batchId}-stable-current-carryover-13`,
  ...
  const receipt = {
    schemaVersion: 1,
    receiptId: 'mathematik-b031-stable-current-carryover-13-v1-20260905',
    purpose: 'Hash-bound bounded carryover of exactly the thirteen current B031 goals confirmed KEEP by both blind rounds; all seven disputed or structural goals remain excluded.',
    ...
    authoringPath: 'stable-current-carryover-13-v1.authoring.json',
    ...
    rationale: [
      'All thirteen claimed bilingual texts and contexts are exact-current and both independent source decisions are KEEP.',
      'The seven named excluded goals are not KEEP/KEEP and receive no resolution, evidence profile, or progress claim from this carryover.',
      'The source campaign contains twenty goals while this bounded carryover contains thirteen; the partial-group compatibility index represents only the claimed stable subset.',
  ...
  console.log(
    `${write ? 'Materialized' : 'Verified'} Mathematics B031 stable-current carryover: strict=${indexEntries.length}/${stableGoalIds.length}; excluded=${excludedGoalIds.length}; index=${indexPath}`,
  )
}
