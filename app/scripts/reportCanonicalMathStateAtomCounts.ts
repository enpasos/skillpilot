...
function countStateRows(): {
  ...
  const rows = tracker.states.map((state): StateAtomicCountRow => {
    ...
    const sek1 = existsSync(sek1ViewPath)
      ? countVisibleAtomicGoalsForStage({
          baseEntry,
      ...
      : (() => {
          const sek1FromGk = countVisibleAtomicGoalsForStage({
            baseEntry,
            jurisdiction: state.jurisdiction,
            courseProfile: 'GK',
            stageTitlePrefix: 'Sekundarstufe I',
          ...
          const sek1FromLk = countVisibleAtomicGoalsForStage({
            baseEntry,
            jurisdiction: state.jurisdiction,
            courseProfile: 'LK',
            stageTitlePrefix: 'Sekundarstufe I',
          ...
          if (sek1FromGk !== sek1FromLk) {
            throw new Error(
              `Sek-I count mismatch for ${state.jurisdiction}: GK=${sek1FromGk}, LK=${sek1FromLk}`,
            )
          }
