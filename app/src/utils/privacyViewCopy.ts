    ...
    ? {
        ...
        sections: [
          ...
          },
          {
            ...
            paragraphs: [
              'If a teacher adds an existing SkillPilot ID to a local class, the teacher’s browser stores the class name, the locally assigned learner name or alias, the permanent SkillPilot ID, and a local copy of the personalization used for the subject views. The teacher view reads the learner profile and learning status directly through SkillPilot’s normal learner endpoints using that ID. When a learning section is first scheduled in the local teacher course plan, a read-only request additionally determines the learner’s current focus, the atomic goals in that scope, and which of them are not yet mastered. The local plan stores those focus and goal IDs, aggregate counts, and the capture time as its fixed planning basis; it stores neither the SkillPilot ID nor individual numeric mastery values in that basis. This workflow creates no separate server-side teacher account, class, authorization record, or membership relationship.',
              'The teacher interface is functionally read-only with respect to learner data: its controls do not change the learner’s personalization, focus, planned learning goals, mastery, or other learner state. A separate local teacher course plan remains editable in the teacher’s browser; it is teacher working data and is not written to the learner record. The plan export omits the learner-derived planning basis, but teacher-entered free text is exported unchanged and may itself contain personal data. This is a user-interface boundary, not a restricted server credential. Under the current identity model, the permanent SkillPilot ID is the bearer secret and full-access key for the learning state. Anyone who knows it can use the normal learner interfaces with the powers attached to that ID; after the local class is removed, the ID remains valid and unchanged. The ID must therefore be shared and stored only with appropriate authorization.',
    ...
    : {
        ...
        sections: [
          ...
          },
          {
            ...
            paragraphs: [
              'Wenn eine Lehrkraft eine bestehende SkillPilot-ID zu einer lokalen Klasse hinzufügt, speichert der Browser der Lehrkraft den Klassennamen, den lokal zugeordneten Namen oder Alias, die dauerhafte SkillPilot-ID und eine lokale Kopie der für die Fachansichten verwendeten Personalisierung. Die Lehreransicht liest Lernprofil und Lernstand mit dieser ID direkt über die normalen SkillPilot-Lernenden-Endpunkte. Wenn erstmals ein Lernabschnitt im lokalen Lehrkraft-Kursplan angelegt wird, ermittelt ein zusätzlicher Nur-Lese-Zugriff den aktuellen Fokus der lernenden Person, die atomaren Ziele dieses Umfangs und welche davon noch nicht gemeistert sind. Der lokale Plan speichert diese Fokus- und Ziel-IDs, Summen und den Erfassungszeitpunkt als feste Planungsgrundlage; in dieser Grundlage werden weder die SkillPilot-ID noch einzelne numerische Mastery-Werte gespeichert. Dieser Ablauf erzeugt keine separate serverseitige Lehrkraft-, Klassen-, Berechtigungs- oder Mitgliedschaftsbeziehung.',
              'Die Lehreroberfläche ist gegenüber den Daten der lernenden Person funktional nur lesend: Ihre Bedienelemente ändern weder deren Personalisierung, Fokus, geplante Lernziele, Mastery-Werte noch sonstigen Lernstand. Ein separater lokaler Kursplan der Lehrkraft bleibt dagegen im Browser der Lehrkraft bearbeitbar; er ist Lehrerarbeitsstand und wird nicht in den Datensatz der lernenden Person geschrieben. Der Planexport lässt die aus dem Lernstand abgeleitete Planungsgrundlage weg; von der Lehrkraft eingegebene Freitexte werden jedoch unverändert exportiert und können selbst personenbezogene Daten enthalten. Diese Grenze liegt in der Benutzeroberfläche, nicht in einem eingeschränkten Server-Zugang. Im derzeitigen Identitätsmodell ist die dauerhafte SkillPilot-ID das Bearer-Geheimnis und der Vollzugriffsschlüssel für den Lernstand. Wer sie kennt, kann die normalen Lernenden-Schnittstellen mit den an diese ID gebundenen Befugnissen verwenden; nach dem Entfernen der lokalen Klasse bleibt die ID gültig und unverändert. Die ID darf deshalb nur mit entsprechender Berechtigung weitergegeben und gespeichert werden.',
