...
for (const [language, markdown] of [['de', de.markdown], ['en', en.markdown]] as const) {
  ...
  assert.match(markdown, /Creative Commons (?:Namensnennung|Attribution) 4\.0 International \(CC BY 4\.0\)/u)
  assert.match(markdown, /https:\/\/github\.com\/enpasos\/skillpilot\/blob\/main\/LICENSE\)/u)
  assert.match(markdown, /https:\/\/github\.com\/enpasos\/skillpilot\/blob\/main\/LICENSES\/CC-BY-4\.0\.txt\)/u)
  assert.match(markdown, /https:\/\/github\.com\/enpasos\/skillpilot\/blob\/main\/LICENSING\.md\)/u)
  assert.match(markdown, /(?:soweit entsprechende Rechte bestehen und wir sie einräumen können|to the extent the relevant rights exist and we can grant them)/u)
  assert.match(markdown, /Apache License, Version 2\.0[^\n]+(?:Software und technische Infrastruktur|Software and technical infrastructure)/u)
  assert.match(markdown, /Apache License, Version 2\.0[^\n]+(?:funktionaler Coach-Instruktionen und Entwicklerdokumentation|functional coach instructions, and developer documentation)/u)
  assert.match(markdown, /CC BY 4\.0[^\n]+(?:Eigene Skill-Landschaften und Bildungsinhalte|Our own skill landscapes and educational content)/u)
  assert.match(markdown, /CC BY 4\.0[^\n]+(?:Curriculumdaten, Skill-Graphen, Lernzielbeschreibungen, fachlicher Zuordnungen, Aufgaben, Lernkarten|curriculum data, skill graphs, learning-goal descriptions, subject mappings, tasks, learning cards)/u)
  assert.match(markdown, /CC BY 4\.0[^\n]+(?:kuratierter Linkbeschreibungen, didaktischer Medien und Whitepaper|curated link descriptions, educational media, and whitepapers)/u)
  assert.match(markdown, /(?:nicht um eine pauschale Wahl zwischen beiden Lizenzen|not a blanket choice between both licenses)/u)
  assert.match(markdown, /(?:lizenziert keine fremden Bestandteile um|does not relicense third-party components)/u)
  assert.match(markdown, /(?:Amtliche Originalquellen, Zitate, verlinkte Anbieterinhalte, Drittanbieter-Abhängigkeiten und Marken|official source documents, quotations, linked provider content, third-party dependencies, and trademarks)/u)
  assert.match(markdown, /(?:Private Lernenden-, Session-, Chat- und sonstige Nutzerdaten sind nicht Teil der Freigabe|Private learner, session, chat, and other user data are not part of this grant)/u)
  assert.match(markdown, /(?:KI-Kennzeichnungen ersetzen keine erforderliche Rechteprüfung|AI labels do not replace any required rights review)/u)
...
assert.doesNotMatch(de.markdown, /Lizenzierung des Quellcodes|Diese Lizenz betrifft die dort bezeichnete Software/u)
assert.doesNotMatch(en.markdown, /Source-code licence|That licence applies to the software identified there/u)
