...
assert(
  deTeacherSupervisionText.includes('Klassennamen')
    ...
    && deTeacherSupervisionText.includes('„Planung wirksam machen“')
    && deTeacherSupervisionText.includes('„Nur dieses Fach aktualisieren“')
    && deTeacherSupervisionText.includes('entweder jede unabhängige Kopie oder keine')
    && deTeacherSupervisionText.includes('weder planbegleitetes Lernen ein noch wählt sie ein Ziel')
    ...
    && deTeacherSupervisionText.includes('schaltet planbegleitetes Lernen ein')
    && deTeacherSupervisionText.includes('erste fällige Ziel mit erfüllten Voraussetzungen')
    && deTeacherSupervisionText.includes('jederzeit pausieren')
    && deTeacherSupervisionText.includes('Fachwechsel parkt das unvollständige bisherige Ziel')
    && deTeacherSupervisionText.includes('ohne Änderung seiner Mastery')
    && deTeacherSupervisionText.includes('gültigen Fachpläne deterministisch')
    && deTeacherSupervisionText.includes('kein Ersatzziel erfunden')
    && deTeacherSupervisionText.includes('allgemeine Autopilot unterdrückt')
    && deTeacherSupervisionText.includes('Kalenderfortschritt allein')
  ...
  'German privacy copy states atomic multi-plan activation, automatic first goal, explicit subject switching, direct-ID access, and learner pause control',
)
assert(
  enTeacherSupervisionText.includes('class name')
    ...
    && enTeacherSupervisionText.includes('“Make planning effective”')
    && enTeacherSupervisionText.includes('“Update this subject only”')
    && enTeacherSupervisionText.includes('either stores every independent copy or none')
    && enTeacherSupervisionText.includes('neither enables plan-guided learning nor selects a goal')
    ...
    && enTeacherSupervisionText.includes('a class reference, teaching coverage, attestations')
    && enTeacherSupervisionText.includes('later local changes are not synchronized automatically')
    && enTeacherSupervisionText.includes('enables plan-guided learning')
    && enTeacherSupervisionText.includes('first due goal whose prerequisites are satisfied')
    && enTeacherSupervisionText.includes('pause that mode at any time')
    && enTeacherSupervisionText.includes('subject switch parks the unfinished previous goal')
    && enTeacherSupervisionText.includes('without changing its mastery')
    && enTeacherSupervisionText.includes('valid subject plans deterministically')
    && enTeacherSupervisionText.includes('no replacement goal is invented')
    && enTeacherSupervisionText.includes('generic Autopilot is suppressed')
    && enTeacherSupervisionText.includes('Calendar progress alone')
  ...
  'English privacy copy states atomic multi-plan activation, automatic first goal, explicit subject switching, direct-ID access, and learner pause control',
)
assert(
