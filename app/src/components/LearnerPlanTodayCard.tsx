...
export const LearnerPlanTodayCard = ({
  ...
  const dailyTargetComplete = plan.metrics.openDueToday === 0
  ...
  return (
    ...
    >
      ...
      {!plan.stale ? (
        <div className="mt-5">
          <LearnerPlanDailyProgress metrics={plan.metrics} subjectLabel={subjectLabel} language={language} />
          {dailyTargetComplete && plan.metrics.dueToday > 0 ? (
            <p className="mt-2 text-sm text-text-secondary">{copy.dailyTargetDoneBody}</p>
          ) : null}
        </div>
      ) : null}

      <details className="mt-4">
        <summary className="min-h-9 cursor-pointer rounded-md text-sm font-medium text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500">
          {copy.detailsAction}
        </summary>
        <p className="mt-2 text-sm text-text-secondary">{copy.includesBacklog}</p>
      ...
      {planModeEnabled && plan.canContinue && navigationAvailable && !staleDataMessage && !actionsDisabled && (
        <div className="mt-5 flex justify-end">
          ...
          >
            {isContinuing ? copy.continueBusy : dailyTargetComplete ? copy.voluntaryContinueAction : copy.continueAction}
          </button>
        </div>
      )}
      ...
      {planModeEnabled && !plan.canContinue && !dailyTargetComplete && !plan.stale && !staleDataMessage && !actionsDisabled && (
        <p
