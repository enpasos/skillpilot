...
const copy = (version?: string) => ({
  de: {
    ...
    betaDescription: CLAUDE_MARKETPLACE_INSTALLATION_ENABLED
      ? 'Empfohlen: Installation über unseren Marketplace. Falls das nicht klappt, lade die Plugin-Datei herunter und in Claude hoch.'
      : 'Lade die aktuelle Plugin-Datei herunter und installiere sie nach dieser Anleitung in Claude Web.',
    loading: 'Aktuelle Plugin-Version wird geladen …',
    loadErrorTitle: 'Die aktuelle Versionsangabe konnte nicht geladen werden.',
    loadErrorText: 'Der Datei-Download und der Versionsvergleich sind bis zum erneuten Laden nicht verfügbar; eine ältere Datei wird nicht als Ersatz angeboten.',
    ...
    marketplaceInstallActions: [
      'Öffne in Claude „Einstellungen“ → „Plugins“ → „Hinzufügen“ oben rechts → „Marketplace hinzufügen“ → „Aus einem Repository hinzufügen“.',
      'Füge die unten stehende Repository-Adresse ein. Lass „Automatisch synchronisieren“ eingeschaltet und wähle „Synchronisieren“.',
      version
    ...
    autosync: 'Lass „Automatisch synchronisieren“ für spätere Marketplace-Updates eingeschaltet. Falls Claude dafür GitHub-Zugriff verlangt, prüfe die angeforderten Rechte; gib nicht pauschal alle Repositorys frei. Möchtest du diese Freigabe nicht erteilen, kannst du die manuelle Updatesuche oder den Datei-Upload verwenden.',
    troubleTitle: 'Hinweise zu Updates und Marketplace-Verwaltung',
    troubleBody: 'Automatische Marketplace-Updates wurden im Betatest bereits auf unabhängigen Claude-Konten beobachtet. Der Zeitpunkt kann variieren. Prüfe die installierte Version unter „Deine Plugins“; der Versionswähler unter „Inhalte“ allein bestätigt sie nicht.',
    ...
    navigationHint: 'Der Link öffnet „Plugins → Entdecken“ in Claude Web, installiert oder aktualisiert aber nichts. Falls er nicht zur Plugin-Ansicht führt: in Claude „Einstellungen“ → „Plugins“ öffnen. Gemeint ist immer das „Hinzufügen“-Menü oben rechts, nicht der Knopf einer beworbenen Plugin-Karte.',
    ...
    requirementsTitle: 'Voraussetzungen und Nutzung',
    ...
    plan: 'Du brauchst vor der Einrichtung ein Claude-Pro-Konto. Der laufende SkillPilot-Betatest nutzt Claude Pro.',
    planDetail: 'Die Claude-App und Voice Mode funktionieren im Betatest. Stockt die Sprachausgabe, warte kurz – bisher spricht Claude danach weiter.',
    install: 'Installiere das Plugin in Claude Web. Danach kannst du mit demselben Konto auch in der Claude-App lernen.',
    independentTitle: 'Unabhängiges Beta-Plugin',
  ...
  en: {
    ...
    betaNotice: version ? 'Claude beta ' + version : 'Claude beta',
    betaDescription: CLAUDE_MARKETPLACE_INSTALLATION_ENABLED
      ? 'Recommended: install through our marketplace. If that does not work, download the plugin file and upload it to Claude.'
      : 'Download the current plugin file and follow this guide to install it in Claude Web.',
    loading: 'Loading the current plugin version …',
    loadErrorTitle: 'The current version information could not be loaded.',
    loadErrorText: 'File download and version comparison are unavailable until you retry successfully; an older file is not offered as a substitute.',
    retry: 'Try again',
    ...
    marketplaceInstallActions: [
      'In Claude, open Settings → Plugins → the Add dropdown at the top right → Add marketplace → Add from a repository.',
      'Paste the repository address below. Leave Automatically sync enabled and select Sync.',
      version
    ...
    autosync: 'Leave Automatically sync enabled for future marketplace updates. If Claude requests GitHub access for this, review the requested permissions; do not grant access to all repositories. If you prefer not to authorize that access, use the manual update check or file upload.',
    troubleTitle: 'About updates and marketplace management',
    troubleBody: 'Automatic marketplace updates have already been observed on independent Claude accounts during the beta. Timing can vary. Check the installed version under Your plugins; the version selector under Contents alone does not confirm it.',
    ...
    navigationHint: 'The link opens Plugins → Discover in Claude Web; it does not install or update anything. If it does not reach that view, open Settings → Plugins in Claude. Always use the Add dropdown at the top right, not the button inside a featured plugin card.',
    ...
    requirementsTitle: 'Requirements and usage',
    supportedPlan: 'Supported beta plan',
    installationSurface: 'Installation',
    age: (minimumAge: number) => 'Only for people aged ' + minimumAge + ' or older.',
    plan: 'You need a Claude Pro account before setup. The ongoing SkillPilot beta uses Claude Pro.',
    planDetail: 'The Claude app and voice mode work in the beta. If speech stalls, wait briefly – in our tests, Claude then continues speaking.',
    install: 'Install the plugin in Claude Web. You can then use the same account to learn in the Claude app.',
    independentTitle: 'Independent beta plugin',
