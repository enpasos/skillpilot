#!/usr/bin/env python3
"""Validate the Bavaria Gymnasium legacy-tree handoff boundary."""
...
REGISTRY_PATH = ROOT / "curricula/DE/Gymnasium/provenance/bavaria-gymnasium-retirement-registry.json"
...
def main() -> None:
    ...
    pattern = rf"{legacy_tree_path}|DE/BY/Gymnasium/"
    ...
    print("Bavaria Gymnasium legacy-tree reference validation")
    print("-------------------------------------------------")
    ...
    if legacy_tree_must_be_absent and legacy_tree_exists:
        print("\nViolation: retired Bavaria Gymnasium legacy tree still exists in the active repo.")
        raise SystemExit(1)
    ...
    print(
        "\nOK: Bavaria Gymnasium legacy-tree references are confined to the explicit "
        "handoff allowlist."
    )
