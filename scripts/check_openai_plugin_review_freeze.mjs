...
export function verifyOpenAiPluginReviewFreeze({
  ...
  const latestAuthorizedTreeExceptionByPath =
    resolveAuthorizedProtectedTreeExceptionChains(
      freeze.protectedTrees,
      expectedAuthorizedRuntimeExceptions,
    );
  ...
  for (const protectedTree of freeze.protectedTrees) {
    ...
    const authorizedException = latestAuthorizedTreeExceptionByPath.get(
      protectedTree.path,
    );
    if (authorizedException) {
      assert.equal(
        protectedTree.sha256,
        authorizedException.protectedTree.submittedSha256,
        `Submitted V1 tree baseline changed: ${protectedTree.path}`,
      );
      assert.equal(
        sha256Tree(treeRoot),
        authorizedException.protectedTree.authorizedSha256,
        `Authorized review exception changed protected tree: ${protectedTree.path}`,
      );
      continue;
    }
    assert.equal(
