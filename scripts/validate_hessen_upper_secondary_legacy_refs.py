#!/usr/bin/env python3
"""Validate the retired Hessen upper-secondary legacy-tree boundary.

After the delete handoff, the old Hessen upper-secondary source tree must stay
absent from the active repo. Remaining textual references are allowed only in a
tightly controlled provenance/handoff set.
"""
...
REGISTRY_PATH = ROOT / "curricula/DE/Gymnasium/provenance/hessen-upper-secondary-retirement-registry.json"
...
def main() -> None:
    ...
    pattern = rf"{legacy_tree_path}|Gymnasiale_Oberstufe"
    ...
    print("Hessen upper-secondary legacy-tree reference validation")
    print("------------------------------------------------------")
    ...
    if legacy_tree_must_be_absent and legacy_tree_exists:
        print("\nViolation: retired Hessen upper-secondary legacy tree still exists in the active repo.")
        raise SystemExit(1)
    ...
    print(
        "\nOK: the retired Hessen upper-secondary legacy tree stays absent, and any "
        "remaining repo references are confined to the explicit handoff allowlist."
    )
