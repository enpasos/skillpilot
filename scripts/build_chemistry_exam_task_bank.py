#!/usr/bin/env python3
"""Build the 2026 Hessen chemistry exam task bank from the landscape JSON."""
...
LANDSCAPE_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_CHEMIE.de.json"
SLOT_MATRIX_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Chemie/slot_matrix.json"
TASK_BANK_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Chemie/task_bank.json"

PHASE_ROOTS = {
    "67876626-924a-4521-983a-cc37006fd67e": ("phase_practice", "E", "reactions_foundations"),
    "a4209619-4b37-42fd-8388-b8dd5489d3cc": ("phase_practice", "Q1", "organic_foundations"),
    "2af02d7b-f1aa-45e7-833f-93011b31cff5": ("phase_practice", "Q2", "biomolecules_polymers"),
    "71be0a89-d69f-4dd4-8830-39e096d137a2": ("phase_practice", "Q3", "equilibria"),
    "8aa34f38-016d-4a8a-8724-f954a74f34ba": ("phase_practice", "Q4", "sustainability_energy"),
}
...
def infer_offer_domain(proposal_id: str | None) -> str:
    mapping = {
        "A": "organic_foundations",
        "B": "biomolecules_polymers",
        "C": "equilibria",
        "D": "buffers_application",
    }
...
def coverage_hints_from_slot(slot: dict) -> list[str]:
    ...
    if slot.get("materialBound"):
        ...
        if slot.get("materialTypes"):
            hints.append("application_or_sustainability_context_present")
    if any(op in INTERPRETATION_OPERATORS for op in slot.get("operatorProfile", [])):
        hints.append("evaluation_component_present")
    if "berechnen" in slot.get("operatorProfile", []):
        hints.append("quantitative_component_present")
...
def main() -> None:
    ...
    for goal in goals:
        ...
        if track == "abi_offer":
            ...
            if slot is None:
                raise ValueError(
                    f"No slot mapping for chemistry offer task {goal['id']} ({goal['title']}) "
    ...
    payload = {
        "examSpecId": "hessen-landesabitur-2026-chemistry",
        "version": 1,
