#!/usr/bin/env python3
"""Build the 2026 Hessen German exam task bank from the landscape JSON."""
...
LANDSCAPE_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_DEUTSCH.de.json"
SLOT_MATRIX_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Deutsch/slot_matrix.json"
TASK_BANK_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Deutsch/task_bank.json"

PHASE_ROOTS = {
    "6624a1c0-4c54-58e0-b636-49c9ec4c653b": ("phase_practice", "E", "language_literary_basics"),
    "913846f9-cb42-5937-a1f9-6de45d8706de": ("phase_practice", "Q1", "epoch_language_nature"),
    "42903f9e-f980-518a-9ba0-f7438d2d2db8": ("phase_practice", "Q2", "public_sphere_drama_resistance"),
    "6e2ff975-c39a-527c-bc59-e41be7cbf97a": ("phase_practice", "Q3", "modernity_language_identity"),
    "e87fdf31-b6c6-5fcf-96cf-0570f67636cc": ("phase_practice", "Q4", "film_present_value_judgement"),
}
...
def infer_proposal_id(title: str) -> str | None:
    match = re.search(r"(?:Pflichtaufgabe|Vorschlag) ([A-D])", title)
    return match.group(1) if match else None


def infer_offer_domain(proposal_id: str | None) -> str:
    mapping = {
        "A": "epoch_language_nature",
        "B": "public_sphere_drama_resistance",
        "C": "modernity_language_identity",
        "D": "materialgestuetztes_schreiben",
    }
...
def coverage_hints_from_slot(slot: dict) -> list[str]:
    ...
    if slot.get("domain") == "materialgestuetztes_schreiben":
        hints.append("materialgestuetztes_schreiben_present")
    if any(op in WRITING_OPERATORS for op in slot.get("operatorProfile", [])):
        hints.append("textbezogenes_schreiben_present")
...
def main() -> None:
    ...
    for goal in goals:
        ...
        if track == "abi_offer":
            ...
            if slot is None:
                raise ValueError(
                    f"No slot mapping for german offer task {goal['id']} ({goal['title']}) "
    ...
    payload = {
        "examSpecId": "hessen-landesabitur-2026-german",
        "version": 1,
