...
def validate_structure(
    ...
    for slot_id, slot in ((slot_id, slot_index[slot_id]) for slot_id in collection.get("slotIds", [])):
...
def validate_bucket_requirements(
    ...
    missing_collection_tasks: list[str] = []
    ...
    for required_collection in requirement.get("mustIncludeCollectionTasks", []):
        ...
        if missing:
            missing_collection_tasks.extend(missing)
            add_finding(
                findings,
                "error",
    ...
    return {
        ...
        "missingCollectionTaskIds": missing_collection_tasks,
    }
