#!/usr/bin/env python3
"""Validate the Hessen 2026 biology exam pipeline artifacts."""
...
DEFAULT_SLOT_MATRIX = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Biologie/slot_matrix.json"
DEFAULT_COVERAGE = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Biologie/coverage_requirements.json"
DEFAULT_TASK_BANK = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Biologie/task_bank.json"
DEFAULT_LANDSCAPE = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_BIOLOGIE.de.json"
...
def validate_task_existence(task_bank_tasks: list[dict], goals_by_id: dict[str, dict], findings: list[dict]) -> None:
    for task in task_bank_tasks:
        goal = goals_by_id.get(task["goalId"])
        if goal is None:
            add_finding(findings, "error", "missing_goal", f"Task {task['goalId']} is not present in the landscape.")
            continue
        if goal["title"] != task["title"]:
            add_finding(
                ...
                "error",
                "stale_title",
                f"Task {task['goalId']} title mismatch: task bank has {task['title']!r}, landscape has {goal['title']!r}.",
            )
        scoring = goal.get("examData", {}).get("scoring", {})
        if task["beTotal"] != scoring.get("maxPoints"):
            add_finding(
                findings,
                "error",
                "stale_be_total",
                f"Task {task['goalId']} BE mismatch: task bank has {task['beTotal']}, landscape has {scoring.get('maxPoints')}.",
            )
...
def main() -> None:
    ...
    coverage_by_collection = {entry["id"]: entry for entry in coverage["collections"]}
    collection_specs, release_errors = resolve_release_collection_specs(goals, slot_matrix["collections"])
    tasks_by_collection = {
    ...
    for message in release_errors:
        add_finding(global_findings, "error", "release_anchor_resolution", message)

    validate_task_existence(task_bank["tasks"], goals_by_id, global_findings)
    validate_collection_membership(task_bank["tasks"], goals, collection_specs, global_findings)

    report_collections: dict[str, dict] = {}
    for collection in collection_specs:
        collection_id = collection["id"]
        tasks = tasks_by_collection.get(collection_id, [])
        findings: list[dict] = []

        structure = validate_structure(collection, tasks, slot_index, findings)
        requirement = coverage_by_collection.get(collection_id, {})
        coverage_summary = {}
        bucket_summary = {}
        quality_summary = {}

        if "unionRequirements" in requirement:
            coverage_summary = validate_union_requirements(requirement["unionRequirements"], tasks, findings)
        if "bucketRequirements" in requirement or "mustIncludeCollectionTasks" in requirement:
            bucket_summary = validate_bucket_requirements(requirement, tasks, tasks_by_collection, findings)
        ...
        release_ready = draft_ready and quality_summary.get("releasePendingFields", 0) == 0

        report_collections[collection_id] = {
    ...
    payload = {
        "examSpecId": slot_matrix["examSpecId"],
    ...
    print("Biology exam pipeline validation summary")
    print("--------------------------------------")
    for collection_id in ("gk_offer_2026", "lk_offer_2026", "gk_master_2026", "lk_master_2026"):
        summary = report_collections.get(collection_id, {})
        print(
            f"{collection_id}: tasks={summary.get('tasksCount', 0)}, "
            f"draftReady={summary.get('draftReady')}, "
            f"releaseReady={summary.get('releaseReady')}, "
            f"findings={len(summary.get('findings', []))}"
        )

    if global_findings and args.fail_on_error:
        raise SystemExit(1)
    if any(entry["severity"] == "error" for entry in global_findings) and args.fail_on_error:
        raise SystemExit(1)
    if any(
        any(finding["severity"] == "error" for finding in collection["findings"])
        for collection in report_collections.values()
    ) and args.fail_on_error:
        raise SystemExit(1)
