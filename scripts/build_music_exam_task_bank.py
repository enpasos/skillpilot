#!/usr/bin/env python3
"""Build the 2026 Hessen music exam task bank from the landscape JSON."""
...
LANDSCAPE_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_MUSIK.de.json"
SLOT_MATRIX_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Musik/slot_matrix.json"
TASK_BANK_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Musik/task_bank.json"

PHASE_ROOTS = {
    "9b2e4ab7-0466-5df8-8242-1a2307fcc0a9": ("phase_practice", "E", "notation_hearing_ensemble"),
    "e45ad724-eb06-5f55-8871-e0ba3d26b9a6": ("phase_practice", "Q1", "form_principles_and_models"),
    "33884d4a-bff3-5316-bbc5-8bdec7f37b74": ("phase_practice", "Q2", "music_media_language_painting"),
    "b9b9caac-b6bd-5bbb-bad1-0044841ccf72": ("phase_practice", "Q3", "modernity_history_and_design"),
    "91f07fb2-4e0d-5bfa-a06b-70cca75ca5af": ("phase_practice", "Q4", "project_curation_reflection"),
}
...
def infer_offer_domain(proposal_id: str | None) -> str:
    mapping = {
        "A": "form_principles_and_models",
        "B": "music_media_language_painting",
        "C": "modernity_history_and_design",
    }
...
def coverage_hints_from_slot(slot: dict) -> list[str]:
    hints: list[str] = list(slot.get("coverageHintIds", []))
    ...
    if any(op in INTERPRETATION_OPERATORS for op in slot.get("operatorProfile", [])):
        hints.append("structured_interpretation_present")
...
def main() -> None:
    ...
    for goal in goals:
        ...
        allowed_tools = infer_allowed_tools(track)
        ...
        if track == "abi_offer":
            ...
            if slot is None:
                raise ValueError(
                    f"No slot mapping for music offer task {goal['id']} ({goal['title']}) "
            ...
            allowed_tools = slot["allowedTools"]
    ...
    payload = {
        "examSpecId": "hessen-landesabitur-2026-music",
        "version": 1,
