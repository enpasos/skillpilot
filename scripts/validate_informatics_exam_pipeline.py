#!/usr/bin/env python3
"""Validate the Hessen 2026 informatics exam pipeline artifacts."""
...
DEFAULT_SLOT_MATRIX = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Informatik/slot_matrix.json"
DEFAULT_COVERAGE = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Informatik/coverage_requirements.json"
DEFAULT_TASK_BANK = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Informatik/task_bank.json"
DEFAULT_LANDSCAPE = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_INFORMATIK.de.json"
...
def enumerate_paths(collection: dict) -> list[list[str]]:
    if collection.get("selectionPaths"):
        return [list(path["slotIds"]) for path in collection["selectionPaths"]]

    selection_groups = collection.get("selectionGroups", [])
    paths: list[list[str]] = [[]]
...
def validate_structure(collection: dict, tasks: list[dict], slot_index: dict[str, dict], findings: list[dict]) -> dict:
    ...
    all_paths = enumerate_paths(collection)
...
def main() -> None:
    ...
    print("Informatics exam pipeline validation summary")
    print("------------------------------------------")
