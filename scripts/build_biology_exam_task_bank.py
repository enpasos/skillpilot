#!/usr/bin/env python3
"""Build the 2026 Hessen biology exam task bank from the landscape JSON."""
...
LANDSCAPE_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_BIOLOGIE.de.json"
SLOT_MATRIX_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Biologie/slot_matrix.json"
TASK_BANK_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Biologie/task_bank.json"

PHASE_ROOTS = {
    "192d4e22-eb3f-4270-affb-a9d6454a7f43": ("phase_practice", "E", "cell_experimental_foundations"),
    "b06ef6f8-1c9f-4f69-8c90-8335b99df525": ("phase_practice", "Q1", "genetics_gene_technology"),
    "68d79715-bbe0-42e8-9209-51f68eadf088": ("phase_practice", "Q2", "neurobiology_behavior"),
    "a84517b2-f18e-4322-b651-252ffec51afc": ("phase_practice", "Q3", "ecology_metabolism_environment"),
    "c8e68e73-7fb3-4c05-a42b-12263cdc3f07": ("phase_practice", "Q4", "sustainability_human_environment"),
}
...
def infer_offer_domain(proposal_id: str | None) -> str:
    mapping = {
        "A": "genetics_gene_technology",
        "B": "ecology_metabolism_environment",
        "C": "neurobiology_behavior_disease",
        "D": "integrative_biology_case",
    }
...
def coverage_hints_from_slot(slot: dict) -> list[str]:
    ...
    if slot.get("materialBound"):
        ...
        if slot.get("materialTypes"):
            hints.append("data_or_material_context_all_proposals")
...
def main() -> None:
    ...
    for goal in goals:
        ...
        if track == "abi_offer":
            ...
            if slot is None:
                raise ValueError(
                    f"No slot mapping for biology offer task {goal['id']} ({goal['title']}) "
    ...
    payload = {
        "examSpecId": "hessen-landesabitur-2026-biology",
        "version": 1,
