#!/usr/bin/env python3
"""Build the 2026 Hessen politics-and-economics exam task bank from the landscape JSON."""
...
LANDSCAPE_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_POLITIKWIRTSCHAFT.de.json"
SLOT_MATRIX_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/PolitikWirtschaft/slot_matrix.json"
TASK_BANK_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/PolitikWirtschaft/task_bank.json"

PHASE_ROOTS = {
    "98e979f7-9ce5-5b70-a8f9-52bcad51bc18": ("phase_practice", "E", "society_ecology_participation"),
    "973fe619-31bc-5f58-993f-a2b9d36f666e": ("phase_practice", "Q1", "constitution_parties_media"),
    "cc4e43a1-5318-5ab2-8964-422f4c83014d": ("phase_practice", "Q2", "policy_competition_labor"),
    "4a9fab92-d9d1-5557-bf76-0d69f5ade628": ("phase_practice", "Q3", "conflict_globalization_environment"),
    "011e75db-5d99-5437-a02d-943798c8266e": ("phase_practice", "Q4", "europe_integration_governance"),
}
...
def infer_offer_domain(proposal_id: str | None) -> str:
    mapping = {
        "A": "constitution_parties_media",
        "B": "policy_competition_labor",
        "C": "conflict_globalization_environment",
    }
...
def coverage_hints_from_slot(slot: dict) -> list[str]:
    ...
    if slot.get("materialBound"):
        ...
        if slot.get("materialTypes"):
            hints.append("institutional_or_data_context_present")
    if any(op in INTERPRETATION_OPERATORS for op in slot.get("operatorProfile", [])):
        hints.append("policy_evaluation_component_present")
...
def main() -> None:
    ...
    for goal in goals:
        ...
        if track == "abi_offer":
            ...
            if slot is None:
                raise ValueError(
                    f"No slot mapping for politics-and-economics offer task {goal['id']} ({goal['title']}) "
    ...
    payload = {
        "examSpecId": "hessen-landesabitur-2026-politics-economics",
        "version": 1,
