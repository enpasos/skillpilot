...
test("the published 1.1.3 release dossier remains immutable history", (t) => {
  const fixture = createFixture(t);
  const path = resolve(fixture, releasePath, "history/1.1.3/marketplace-publication.json");
  writeFileSync(path, `${readFileSync(path, "utf8")}\n`, "utf8");
  assert.match(verifyClaudePluginV1Release({ repositoryRoot: fixture }).errors.join("\n"),
    /Historical plugin release file changed: 1\.1\.3\/marketplace-publication\.json/u);
});

test("the published 1.1.5 release dossier remains immutable history", (t) => {
  const fixture = createFixture(t);
  const path = resolve(fixture, releasePath, "history/1.1.5/marketplace-publication.json");
  writeFileSync(path, `${readFileSync(path, "utf8")}\n`, "utf8");
  assert.match(verifyClaudePluginV1Release({ repositoryRoot: fixture }).errors.join("\n"),
    /Historical plugin release file changed: 1\.1\.5\/marketplace-publication\.json/u);
});

test("the published 1.1.8 release dossier remains immutable history", (t) => {
  const fixture = createFixture(t);
  const path = resolve(fixture, releasePath, "history/1.1.8/marketplace-publication.json");
  writeFileSync(path, `${readFileSync(path, "utf8")}\n`, "utf8");
  assert.match(verifyClaudePluginV1Release({ repositoryRoot: fixture }).errors.join("\n"),
    /Historical plugin release file changed: 1\.1\.8\/marketplace-publication\.json/u);
});

test("the published 1.1.4 release dossier remains immutable history", (t) => {
  const fixture = createFixture(t);
  const path = resolve(fixture, releasePath, "history/1.1.4/marketplace-publication.json");
  writeFileSync(path, `${readFileSync(path, "utf8")}\n`, "utf8");
  assert.match(verifyClaudePluginV1Release({ repositoryRoot: fixture }).errors.join("\n"),
    /Historical plugin release file changed: 1\.1\.4\/marketplace-publication\.json/u);
});
...
test("the 1.1.0 replacement dossier remains immutable history", (t) => {
  const fixture = createFixture(t);
  const historicalPath = resolve(fixture, releasePath, "history/1.1.0/direct-install-beta.json");
  writeFileSync(historicalPath, `${readFileSync(historicalPath, "utf8")}\n`, "utf8");

  const result = verifyClaudePluginV1Release({ repositoryRoot: fixture });

  assert.match(
    result.errors.join("\n"),
    /Historical plugin release file changed: 1\.1\.0\/direct-install-beta\.json/u,
  );
});

test("the published 1.1.1 release dossier remains immutable history", (t) => {
  const fixture = createFixture(t);
  const historicalPath = resolve(fixture, releasePath, "history/1.1.1/marketplace-publication.json");
  writeFileSync(historicalPath, `${readFileSync(historicalPath, "utf8")}\n`, "utf8");
  ...
  assert.match(
    result.errors.join("\n"),
    /Historical plugin release file changed: 1\.1\.1\/marketplace-publication\.json/u,
  );
});
