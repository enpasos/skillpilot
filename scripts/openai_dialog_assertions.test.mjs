...
test('P4 saves the passed submission before feedback and waits only before successor content', () => {
  const c = getCase('P4')
  ...
  const delayedSave = base(c)
  insert(delayedSave, 'submission', evaluation)
  insert(delayedSave, 'closure', { ...mastery, turnId: 'closure' })
  assert.equal(failed(evaluateDialogCase(c, delayedSave), 'exam-immediate-save'), true)
  const feedbackBeforeSave = base(c)
  insert(feedbackBeforeSave, 'submission', evaluation, reply('submission', 'Bestanden!'), mastery)
  assert.equal(failed(evaluateDialogCase(c, feedbackBeforeSave), 'exam-immediate-save'), true)
  const saveBeforeEvaluation = base(c)
  insert(saveBeforeEvaluation, 'submission', mastery, evaluation)
  assert.equal(failed(evaluateDialogCase(c, saveBeforeEvaluation), 'exam-score'), true)
  ...
  const oldGoalImage = base(c)
  insert(oldGoalImage, 'submission', tool('submission', 'render_skillpilot_goal_visualization', {}, { goalId: 'g1', expectedStateVersion: 1 }))
  assert.equal(failed(evaluateDialogCase(c, oldGoalImage), 'feedback-image-gate'), true)
