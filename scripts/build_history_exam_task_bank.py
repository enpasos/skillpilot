#!/usr/bin/env python3
"""Build the 2026 Hessen history exam task bank from the landscape JSON."""
...
LANDSCAPE_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/json/DE_HES_S_GYM_2_GESCHICHTE.de.json"
SLOT_MATRIX_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Geschichte/slot_matrix.json"
TASK_BANK_PATH = ROOT / "curricula/DE/HE/Kultusministerium/Gymnasiale_Oberstufe/abi/Geschichte/task_bank.json"

PHASE_ROOTS = {
    "87874470-47e9-5352-92ca-4657ce61b1f0": ("phase_practice", "E", "historical_orientation_revolution"),
    "260e8176-a4b1-5791-a4a6-fb0c0c151ec8": ("phase_practice", "Q1", "constitution_revolution_nation"),
    "c83b81b5-9ff6-5151-b98e-01603563f643": ("phase_practice", "Q2", "democracy_dictatorship_revolution"),
    "8cd2c950-34a3-5e5a-bcdb-65afdfcfdc22": ("phase_practice", "Q3", "cold_war_unity_world_order"),
    "4906111b-bdc8-54b9-bfa8-e81f82eed130": ("phase_practice", "Q4", "memory_culture_historical_politics"),
}
...
def infer_proposal_id(title: str) -> str | None:
    match = re.search(r"Vorschlag ([A-C])", title)
    return match.group(1) if match else None


def infer_offer_domain(proposal_id: str | None) -> str:
    mapping = {
        "A": "constitution_revolution_nation",
        "B": "democracy_dictatorship_revolution",
        "C": "cold_war_unity_world_order",
    }
...
def coverage_hints_from_slot(slot: dict) -> list[str]:
    ...
    if slot.get("materialBound"):
        ...
        hints.append("source_based_argumentation_present")
    if any(op in JUDGEMENT_OPERATORS for op in slot.get("operatorProfile", [])):
        hints.append("historical_judgement_component_present")
...
def main() -> None:
    ...
    for goal in goals:
        ...
        if track == "abi_offer":
            ...
            if slot is None:
                raise ValueError(
                    f"No slot mapping for history offer task {goal['id']} ({goal['title']}) "
    ...
    payload = {
        "examSpecId": "hessen-landesabitur-2026-history",
        "version": 1,
