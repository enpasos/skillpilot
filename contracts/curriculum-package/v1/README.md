...
DPK-001 established the manifest/profile kernel; DPK-002 adds package-local discovery, offline schema resolution, and a fail-closed readiness vocabulary. DPK-003 freezes the cross-variant semantic proof contract. DPK-004 completes the strict unpacked Mathematik release model: DPK-004a established Runtime closure, and DPK-004b adds authoritative publication evidence. DPK-005a closes package-level semantic bindings, the complete offline schema trust set, the deterministic ZIP32 primitive, and human-review gates. DPK-005b is now technically implemented: it materializes that model as a reproducible real ZIP and validates the finished carrier independently. Its targeted checks are green; the complete repository `./run_ci.sh` for the step is still outstanding.
...
The existing subject exporter still produces the explicitly marked legacy subject-export format. It is not conformant with `full-standalone-v1`; successful legacy export and roundtrip checks remain `not-ready-legacy`. For the new DPK-005b staging ZIP, inventory, Runtime catalog, offline schema catalog, hard-reference closure, content digest and binary bytes are implemented and independently checked. Readiness nevertheless remains `not-ready-incomplete`: DPK-006/007 have not yet supplied the hermetic package-only consumer proof, and open redistribution decisions still block publication. The inner package-manifest and `full-standalone-v1` profile cover only the JSON runtime variant; the external DPK-003 proof contracts cover both variants, while the FWU-OWL inner manifest/profile remains a later contract step.
