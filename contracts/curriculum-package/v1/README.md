...
Provision the pinned ontology tools once, then run the heavyweight real FWU-OWL gate:
...
Only provisioning may use the network. The conformance run checks exact Python dependency, ROBOT 1.9.10, HermiT 1.4.5.456 and Amazon Corretto 25.0.2.10.1 identities before validating offline. It writes the canonical receipt to `tmp/curriculum-release-model/fwu-owl-validation/fwu-owl-package-validation-report.json`. This multi-gigabyte gate is intentionally separate from ordinary CI; CI runs its 40-guarantee validator self-test and verifies the pinned toolchain contract.
