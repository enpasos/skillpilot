...
- `fixtures/fwu-owl-package/` and `scripts/validate_curriculum_fwu_owl_package_contracts.py` exercise one externally anchored contract set, truthful early invalid/error receipts, 93 fail-closed semantic mutations, duplicate-key rejection, and non-finite-number rejection independently of the future exporter. The dual-release validator adds 61 cross-variant mutations.
...
The existing subject exporter still produces the explicitly marked legacy subject-export format. It is not conformant with `full-standalone-v1`; successful legacy export and roundtrip checks remain `not-ready-legacy`. For the new staging ZIP, inventory, Runtime catalog, offline schema catalog, hard-reference closure, content digest and binary bytes are implemented and independently checked. DPK-006a additionally proves exact read-only loading from a prevalidated store, DPK-006b/006c supply safe provisioning and package-authoritative services, DPK-007 proves the hermetic whole-application consumer path, and DPK-007a freezes its complete content and Core/registry lane semantics. Readiness nevertheless remains `not-ready-incomplete`, because the 621 fachliche image reviews, 760 redistribution reviews, and 479 source-text reviews remain human gates. The inner package-manifest and `full-standalone-v1` profile cover only the JSON runtime variant; the external DPK-003 proof contracts cover both variants, while DPK-008 supplies the FWU-OWL inner manifest/profile and isolated reverse compiler.
