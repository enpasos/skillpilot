...
class ContentSelectionControllerHttpTest {
    ...
    private static final String GRANTS = "{\"" + LEARNER_A + "\":\""
            + ChampionPracticeFingerprint.digest(KEY) + "\"}";
    ...
    private MockMvc controller(boolean enabled) {
        return MockMvcBuilders.standaloneSetup(new ContentSelectionController(
                        new ContentConfigurationAuthorization(enabled, GRANTS),
    ...
    @Test
    void learnerIdAloneNeverAuthorizesAWriteAndErrorsAreNotCacheable() throws Exception {
    ...
    void unsupportedInputIsRejectedBeforeAnyPersistenceOrActivity(String request) throws Exception {
        mvc.perform(put("/api/ui/learners/{id}/content-selection", LEARNER_A)
                        .header(ContentConfigurationAuthorization.HEADER, KEY)
    ...
    @Test
    void disabledPilotReturns404WithoutReadingLearnerState() throws Exception {
    ...
    @Test
    void selectionReadIsNoStoreAndDoesNotExposeConfigurationCapabilityOrLearnerIdentity() throws Exception {
        ...
        var result = mvc.perform(get("/api/ui/learners/{id}/content-selection", LEARNER_A).queryParam("lang", "en"))
                ...
                .andReturn();
        assertThat(result.getResponse().getContentAsString()).doesNotContain(KEY, GRANTS, LEARNER_A, LEARNER_B);
    ...
    void onlyGoalsInTheFilteredLearnerClosureCanResolveMaterials() throws Exception {
        ...
        var result = mvc.perform(get("/api/ui/learners/{id}/content-materials", LEARNER_A)
                ...
                .andReturn();
        assertThat(result.getResponse().getContentAsString()).doesNotContain(KEY, LEARNER_A, LEARNER_B);
    ...
    @Test
    void validWriteUsesExactlyTheAuthorizedLearnerAndCannotReuseGrantForAnotherProfile() throws Exception {
        when(selections.update(LEARNER_A, KEY, 0, List.of("package-one")))
                .thenReturn(new ContentSelectionService.Selection(1, Set.of("package-one")));
        when(catalog.packages("de")).thenReturn(List.of());
        var result = mvc.perform(put("/api/ui/learners/{id}/content-selection", LEARNER_A)
                        .header(ContentConfigurationAuthorization.HEADER, KEY)
                        .contentType(MediaType.APPLICATION_JSON).content(VALID_REQUEST))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CACHE_CONTROL, "no-store"))
        ...
        mvc.perform(put("/api/ui/learners/{id}/content-selection", LEARNER_B)
                        .header(ContentConfigurationAuthorization.HEADER, KEY)
                        .contentType(MediaType.APPLICATION_JSON).content(VALID_REQUEST))
                .andExpect(status().isForbidden());
        verify(selections).update(LEARNER_A, KEY, 0, List.of("package-one"));
