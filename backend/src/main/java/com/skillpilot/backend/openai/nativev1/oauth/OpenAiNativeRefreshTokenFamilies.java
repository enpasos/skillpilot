...
public final class OpenAiNativeRefreshTokenFamilies {
    private final JdbcOperations jdbc;
    private final TransactionTemplate transaction;
    private final TransactionTemplate refreshTransaction;

    public OpenAiNativeRefreshTokenFamilies(JdbcOperations jdbc, PlatformTransactionManager manager) {
    ...
    void record(OAuth2Authorization authorization) {
        ...
        if (families.isEmpty()) {
            jdbc.update("INSERT INTO openai_native_refresh_family (authorization_id, registered_client_id, revoked, expires_at) VALUES (?, ?, ?, ?)",
        ...
        String originalClient = jdbc.queryForObject("SELECT registered_client_id FROM openai_native_refresh_family WHERE authorization_id = ?",
        ...
        List<Boolean> known = jdbc.query("SELECT consumed FROM openai_native_refresh_history WHERE token_hash = ? AND authorization_id = ?",
                (rs, row) -> rs.getBoolean(1), hash, familyId);
        if (known.isEmpty()) {
            jdbc.update("UPDATE openai_native_refresh_history SET consumed = ? WHERE authorization_id = ?", true, familyId);
            jdbc.update("INSERT INTO openai_native_refresh_history (token_hash, authorization_id, consumed) VALUES (?, ?, ?)", hash, familyId, false);
    ...
    boolean permits(OAuth2Authorization authorization) {
        ...
        List<Boolean> state = jdbc.query("SELECT revoked FROM openai_native_refresh_family WHERE authorization_id = ?",
    ...
    void revoke(String familyId) {
        lock(familyId);
        jdbc.update("UPDATE openai_native_refresh_family SET revoked = ? WHERE authorization_id = ?", true, familyId);
    }
    ...
    public void cleanupExpiredFamilies() {
        ...
        List<String> expired = jdbc.query("SELECT authorization_id FROM openai_native_refresh_family WHERE expires_at < CURRENT_TIMESTAMP ORDER BY expires_at LIMIT 200",
                (rs, row) -> rs.getString(1));
        for (String familyId : expired) {
            transaction(() -> {
                lock(familyId);
                Integer eligible = jdbc.queryForObject("SELECT COUNT(*) FROM openai_native_refresh_family WHERE authorization_id = ? AND expires_at < CURRENT_TIMESTAMP",
                        Integer.class, familyId);
                if (eligible != null && eligible == 1) {
                    jdbc.update("DELETE FROM openai_native_refresh_history WHERE authorization_id = ?", familyId);
                    jdbc.update("DELETE FROM openai_native_refresh_family WHERE authorization_id = ?", familyId);
                }
                return null;
            });
        }
    ...
    Authentication refresh(String token, String registeredClientId, Supplier<Authentication> exchange) {
        Authentication result = refreshTransaction.execute(status -> {
            ...
            List<String> ids = jdbc.query("SELECT h.authorization_id FROM openai_native_refresh_history h JOIN openai_native_refresh_family f ON f.authorization_id = h.authorization_id WHERE h.token_hash = ? AND f.registered_client_id = ?",
            ...
            Boolean consumed = jdbc.queryForObject("SELECT consumed FROM openai_native_refresh_history WHERE token_hash = ? AND authorization_id = ?",
                    Boolean.class, tokenHash, familyId);
            if (Boolean.TRUE.equals(consumed)) {
                revoke(familyId);
                com.skillpilot.backend.oauth.OAuthProfileDiagnostics.markProfile("chatgpt-native-cimd-public");
    ...
    private List<Boolean> lock(String familyId) {
        return jdbc.query("SELECT revoked FROM openai_native_refresh_family WHERE authorization_id = ? FOR UPDATE",
