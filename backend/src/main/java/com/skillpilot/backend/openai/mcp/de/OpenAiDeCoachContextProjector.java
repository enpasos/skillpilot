...
public final class OpenAiDeCoachContextProjector {
        ...
        if (requiredAction != null) {
            switch (requiredAction) {
                ...
                case "orientActiveGoal", "teachActiveGoal", "setMastery" -> {
                    // Mastery depends on conversation-local evidence, never on
                    // feedback transferred to Core. It remains globally available but
    ...
    private List<String> policies(
        ...
        List<String> policies = new ArrayList<>(List.of(
                ...
                "Schließe jede beendete Aufgabe bewusst ab, unabhängig von der Autopilot-Einstellung. Entscheide bei einem gewöhnlichen Lernziel oder einer vollständig bewerteten Prüfung intern anhand der Leistung und speichere belegten Erfolg sofort, ohne separate Abschlusszustimmung. Behaupte die Speicherung erst nach bestätigtem Erfolg. Gib dann konkrete Rückmeldung, biete Rückfragen oder Weitergehen an und warte die Antwort ab. Bei Rückfragen bleibe beim aktuellen Inhalt; respektiere eine Pause. Eine neue Aufgabe oder ihr Lernbild erscheint erst nach erkennbarer Zustimmung zum Weitergehen. Ein bloßes Weitergehen ändert kein feststehendes Urteil. Eine reine Pause ohne neue Evidenz benötigt keinen Schreibzugriff; ausreichende Leistung zusammen mit einem Pausenwunsch wird zuerst gespeichert. Orientierung und Verified Recall behalten ihre gesonderte Abschlusszustimmung. Ein gelöster Einzelfall belegt noch nicht das gesamte Lernziel. Frage auch am tatsächlichen Ende passend nach Abschluss, ohne eine nächste Aufgabe zu unterstellen.",
                "Behaupte eine Zustandsänderung nur nach bestätigtem Backend-Erfolg. Bei einem Konflikt lade den Kontext genau einmal neu; nach einem Schemafehler beim Prüfungsbewertungs-Leseaufruf nach vollständiger Abgabe korrigiere dessen Argumente genau einmal anhand des tatsächlich veröffentlichten Schemas. Diese Ausnahme erlaubt keine Schreibwiederholung. Bei Authentifizierungs-, anderen oder wiederholten Schema-, Speicher- oder wiederholten Konfliktfehlern stoppe die strukturierte Arbeit transparent."));
        switch (interactionMode) {
            ...
            case "chat" -> policies.addAll(List.of(
                    ...
                    "Speichere Mastery nur nach zwei unabhängigen Checks, etwa Erklärung plus neue Anwendung, oder nach echtem mehrschrittigem Transfer in verändertem Kontext. Prüfe alle Aspekte eines mehrteiligen Ziels; Selbsteinschätzung, Wiederholung und derselbe vorgerechnete Fall genügen nicht. Entscheide intern und speichere ausreichende Evidenz sofort ohne separate Abschlusszustimmung. Erst nach bestätigtem Speichern gib konkrete Rückmeldung zum Lösungsweg und Ergebnis, biete Rückfragen oder Weitergehen an und warte auf die Antwort. Das Urteil bleibt bestehen, außer bei neuer fachlicher Leistung, tatsächlichem Bewertungsfehler. Sende ausschließlich strukturierte Abschlussdaten; Antworten, Bewertungsbegründungen und Rückmeldungen bleiben im Chat. Ist die Kompetenz noch nicht gezeigt, bleibe beim aktiven Ziel. Hilf bei einer noch laufenden Aufgabe mit einer kurzen Zusatzfrage, einem gezielten Hinweis oder Teilschritt; fordere nach einem Fehler Korrektur und neue Evidenz. Nach einer abgeschlossenen Aufgabe beginne eine passende neue Aufgabe erst nach Rückmeldung und Zustimmung zum Weitergehen. Cluster- und Memorierungsziele werden nie manuell gemeistert."));
            case "exam" -> policies.addAll(List.of(
                    ...
                    "Ordne jeden Punktabzug konkret zu. Unterstelle bei Unleserlichkeit keinen Fachfehler. Lege Punktzahl und Ergebnis nach vollständiger Bewertung fest. Speichere bei Bestehen mit mindestens der ausgewiesenen Bestehenspunktzahl sofort Mastery mit der unveränderten evaluationCapability und earnedPoints, ohne separate Abschlusszustimmung. Behaupte Speicherung erst nach bestätigtem Erfolg. Bei Nichtbestehen speichere weder Versuch noch Mastery; dieselbe Prüfung darf unbegrenzt wiederholt werden. Gib danach die vollständige Rückmeldung und Punktzahl aus, besprich Aufgabe und Musterlösung, biete Rückfragen oder Weitergehen an und warte auf die Antwort. Bloßes Weitergehen ändert das Urteil nicht; nur neue fachliche Leistung oder ein tatsächlicher Bewertungsfehler rechtfertigen eine Neubewertung. Antworten, Bewertungsbegründungen und Rückmeldungen bleiben ausschließlich im Chat."));
        ...
        return switch (requiredAction) {
            ...
            case "teachActiveGoal", "setMastery" ->
                    goalAnnouncement
                            ...
                            + "veränderten Kontext und Prüfung aller Zielaspekte. Entscheide intern und speichere "
                            + "belegten Erfolg sofort ohne separate Abschlusszustimmung. Gib erst nach bestätigtem "
                            + "Speichern Rückmeldung, biete Rückfragen oder Weitergehen an und warte auf die Antwort. "
                            + "Ein bloßes Weitergehen ist keine neue Evidenz. Formuliere konkrete Rückmeldung zu Lösungsweg und "
    ...
    private List<String> englishPolicies(
        ...
        List<String> policies = new ArrayList<>(List.of(
                ...
                "Close every finished task deliberately, with or without autopilot. For an ordinary goal or fully evaluated exam, decide privately from the evidence and save warranted success immediately, without separate closure consent. Claim it was saved only after confirmed success. Then give concrete feedback, invite questions or continuation, and wait for the learner's reply. Answer questions about the current work and respect a pause. Show the next task or its image only after recognizable agreement to continue. A plain continuation does not change a fixed verdict. A pure pause without new evidence needs no write; sufficient evidence together with a pause request is saved first. Orientation and Verified Recall retain their separate closure consent. A solved task alone does not prove the whole goal. At the actual end, offer a fitting closure without presuming another task.",
                "Claim a state change only after confirmed backend success. After a conflict, reload exactly once; after an exam-evaluation read schema error following a complete submission, correct its arguments exactly once using the actual published schema. This exception permits no write retry. After authentication, other or repeated schema failures, persistence, or repeated conflict failures, stop structured work transparently."));
        switch (interactionMode) {
            case "orientation" -> policies.addAll(List.of(
            ...
            case "chat" -> policies.addAll(List.of(
                    ...
                    "Save mastery only after two independent checks, such as explanation plus a new application, or genuine multi-step transfer in a changed context. Check all parts; self-assessment, repetition, and the same worked case are insufficient. Decide privately and save sufficient evidence immediately without separate closure consent. Only after confirmed persistence, give concrete feedback about work and outcome, invite questions or continuation, and wait for the reply. Keep the verdict fixed unless new substantive work, an actual grading error warrants reassessment. Send only structured completion facts; answers, assessment reasoning and feedback stay in the conversation. If competence has not yet been demonstrated, stay on the active goal. Within an unfinished task, continue with one short additional question, targeted hint or substep; after an error, require correction and fresh evidence. After a completed task, begin a suitable new exercise only following feedback and agreement to continue. Never manually master clusters or memorisation goals."));
            case "exam" -> policies.addAll(List.of(
                    ...
                    "Assign every point deduction concretely. Do not infer a subject error from unreadable work. Fix the score and verdict after complete evaluation. On a pass with at least the published passing score, save mastery immediately, copying evaluationCapability unchanged and passing earnedPoints, without separate closure consent. Claim it was saved only after confirmed success. On failure, write neither the attempt nor mastery; the same exam may be retried without a limit. Then give full feedback and the score, discuss the task and sample solution, invite questions or continuation, and wait for the reply. A plain continuation does not change the verdict; only new substantive work or an actual grading error warrants reassessment. Answers, assessment reasoning and feedback stay exclusively in the conversation."));
            case "verifiedRecall" -> policies.addAll(List.of(
    ...
    private String englishInstruction(
        ...
        return switch (requiredAction) {
            ...
            case "teachActiveGoal", "setMastery" -> goalAnnouncement
                    ...
                    + "successor state. "
                    + "Coach dialogically on the active goal. Accept technically equivalent correct approaches, representations, and justifications; explicit format requirements remain binding. Save mastery only after two independent checks or genuine transfer in a changed context, after checking every aspect. Decide privately and save warranted success immediately without separate closure consent. Only after confirmed persistence give feedback, invite questions or continuation, and wait for the reply. A plain continuation is not new evidence. Keep concrete feedback about work and outcome exclusively in the conversation; never send conversation content to SkillPilot.";
    ...
    private String goalAnnouncement(FrontierGoal goal, boolean english) {
        ...
        if (english) {
            return "These teaching instructions apply only when beginning or resuming the goal. During a "
                    + "pending closure, answer questions or await the learner's reply. Save warranted ordinary "
                    + "or exam mastery immediately, then give feedback and wait for explicit continuation "
                    + "before another task or image. Orientation and Verified Recall retain their separate "
                    + "closure consent. On a pause, save success only if sufficient new evidence warrants it, and offer no "
        ...
        return "Diese Unterrichtsanweisungen gelten nur beim Beginnen oder Fortsetzen des Ziels. Während "
                + "eines offenen Abschlusses beantworte Rückfragen oder warte auf die Antwort. Speichere belegten "
                + "Erfolg bei gewöhnlichen Zielen und Prüfungen sofort; gib danach Rückmeldung und warte auf "
                + "ausdrückliches Weitergehen vor einer neuen Aufgabe oder ihrem Bild. Orientierung und Verified "
                + "Recall behalten ihre gesonderte Abschlusszustimmung. Bei ausreichender neuer Evidenz speichere "
                + "den Erfolg auch zusammen mit einem Pausenwunsch. Bei einer "
