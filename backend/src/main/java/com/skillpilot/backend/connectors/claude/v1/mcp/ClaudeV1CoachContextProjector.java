...
public class ClaudeV1CoachContextProjector {
    ...
    static final String GOAL_VISUALIZATION_PRESENTATION_INSTRUCTION =
            "Required presentation only when beginning or resuming teaching: form the pair from "
                    ...
                    + "choice to continue, do not call the renderer or present the next task. For an ordinary "
                    + "goal or passed exam, perform the warranted completion write immediately after deciding, "
                    + "then give feedback and wait for explicit continuation before rendering from its returned "
                    + "context; never render a stale image from the pre-write context. Orientation and Verified "
                    + "Recall retain their separate closure rules. "
    ...
    private Map<String, Object> dailyPlanGuidance(String state) {
        String instruction = switch (state) {
            case "continue" -> "Output 'text' verbatim when reporting plan status, at most once per response, "
                    ...
                    + "goal directly when teaching is authorized. After ordinary or exam mastery is saved, "
                    + "give feedback and await an explicit request to continue before another task or goal. "
                    + "After task-only feedback, also await the learner's answer. A clear request for "
                    + "a different available subject takes priority. A pure status-only question or pure pause "
                    + "request without new sufficient evidence needs no new exercise or write. If an answer "
                    + "also proves completion, save positive mastery first and then honor the pause.";
